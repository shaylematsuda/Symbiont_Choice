...
#HERE IS THE ISSUE: you are using only the df with larva where all 4 amped and you don't have to. For ddPCR any clade that amped shoudl be included. figure out which df to use and you might want to use 1619 line (btw you fixed this)
...
ALLFullscopehemo2<-subset(ALLFullscopehemo2, select = -c(cells.larvae) )# drop cells.larvae col

All_binomial<-rbind(ALLFullscopehemo2,RelA_binaryb_2) #combine for the ALL BINOMIAL ONLY DF
...
dispersion_glmer(Binary_mod) # no overdispersion  .77
  ...
  cld(emm) 
    ...
    emm<-emmeans(Binary_mod,  ~ Symbiont*Day|Temp)
cld(emm)
...
Stats: By Temperature: at each temp, which clade infects the best? (dont need)
```{r}      
#by Temp:   
...
#27C
  ...
  dispersion_glmer(Binary_27C_mod)  #no overdispersion 
    emm<-emmeans(Binary_27C_mod,  ~ Symbiont)
  pairs(emm)
  cld(emm)  
...
# Monoculture vs Polyculture by clade: Binomial TAKE 2 -PINK and BLUE binomial:  
...
CELL DENSITY
# Monoculture and Polucluture (total, not by genus): Plot comparing cell counts of individual infection vs total for poly
Fig S6  
...
#organize Single df
SingleCounts2<-rbind(C1Full_no0,D1Full_no0, F1Full_no0, G3Full_no0)
...
CompetitionSingle<-subset(CompetitionSingle, cells.larvae_round>0) #remove 0s
...
CompetitionSingle_mean<-na.omit(CompetitionSingle_mean) #remove NAs
...
#combine all plots
grid.arrange(CompetitionSingle_27_plot, CompetitionSingle_30_plot, CompetitionSingle_31_plot,  nrow=1)
                                  
 
#stats
#df CompetitionSingle
CompetitionSingle_mod<-lmer(cells.larvae_log~Day*Symbiont*Temp+(1|code),CompetitionSingle)
anova(CompetitionSingle_mod)
qqPlot(residuals(CompetitionSingle_mod))   # good
  emm<-emmeans(CompetitionSingle_mod,  ~ Symbiont|Day|Temp, adjust="Tukey") 
cld(emm)
pairs(emm)

#27 and 30 only

CompetitionSingle_27C30C<-subset(CompetitionSingle, Temp=="27"|Temp=="30")

CompetitionSingle_27C30C_mod<-lmer(cells.larvae_log~Day*Symbiont*Temp+(1|code),CompetitionSingle_27C30C)
anova(CompetitionSingle_27C30C_mod)
qqPlot(residuals(CompetitionSingle_27C30C_mod))   # good
  emm<-emmeans(CompetitionSingle_27C30C_mod,  ~ Symbiont|Day|Temp, adjust="Tukey") 
cld(emm)
pairs(emm)
...
CompetitionSingle_D3D7<-subset(CompetitionSingle, Day=="3"|Day=="7")

CompetitionSingle_D3D7_mod<-lmer(cells.larvae_log~Day*Symbiont*Temp+(1|code),CompetitionSingle_D3D7)
anova(CompetitionSingle_D3D7_mod)
qqPlot(residuals(CompetitionSingle_D3D7_mod))   # good
  emm<-emmeans(CompetitionSingle_D3D7_mod,  ~ Symbiont|Day|Temp, adjust="Tukey") 
cld(emm)
pairs(emm)
...
#27 
CompetitionSingle_T27<-subset(CompetitionSingle, Temp==27)
CompetitionSingle_T27_mod<-lmer(cells.larvae_log~Day*Symbiont+(1|code),CompetitionSingle_T27)
anova(CompetitionSingle_T27_mod)

qqPlot(residuals(CompetitionSingle_T27_mod))   # good
  emm<-emmeans(CompetitionSingle_T27_mod,  ~ Symbiont|Day, adjust="Tukey") 
cld(emm)
pairs(emm)

#30 
CompetitionSingle_T30<-subset(CompetitionSingle, Temp==30)
CompetitionSingle_T30_mod<-lmer(cells.larvae_log~Day*Symbiont+(1|code),CompetitionSingle_T30)
anova(CompetitionSingle_T30_mod)
qqPlot(residuals(CompetitionSingle_T30_mod))   # good
  emm<-emmeans(CompetitionSingle_T30_mod,  ~ Symbiont|Day, adjust="Tukey") 
cld(emm)
pairs(emm)


#31
CompetitionSingle_T31<-subset(CompetitionSingle, Temp==31)
CompetitionSingle_T31_mod<-lmer(cells.larvae_log~Day*Symbiont+(1|code),CompetitionSingle_T31)
anova(CompetitionSingle_T31_mod)
qqPlot(residuals(CompetitionSingle_T31_mod))   # good
  emm<-emmeans(CompetitionSingle_T31_mod,  ~ Symbiont|Day, adjust="Tukey") 
cld(emm)
pairs(emm)
```
...
# New FIg 1 Plot combo counts of mono and poly and pink and blue
```{r}  
 
#combine all plots
grid.arrange(CompetitionSingle_27_plot, CompetitionSingle_30_plot, CompetitionSingle_31_plot,  CountsCompare2_C1_plot27, CountsCompare2_C1_plot30, CountsCompare2_C1_plot31, CountsCompare2_D1_plot27,CountsCompare2_D1_plot30, CountsCompare2_D1_plot31,CountsCompare2_F1_plot27,CountsCompare2_F1_plot30,CountsCompare2_F1_plot31,CountsCompare2_G3_plot27,CountsCompare2_G3_plot30,CountsCompare2_G3_plot31,nrow=5)



InfectRaw_Fig1<-plot_grid(CompetitionSingle_27_plot, CompetitionSingle_30_plot, CompetitionSingle_31_plot,  CountsCompare2_C1_plot27, CountsCompare2_C1_plot30, CountsCompare2_C1_plot31, CountsCompare2_D1_plot27,CountsCompare2_D1_plot30, CountsCompare2_D1_plot31,CountsCompare2_F1_plot27,CountsCompare2_F1_plot30,CountsCompare2_F1_plot31,CountsCompare2_G3_plot27,CountsCompare2_G3_plot30,CountsCompare2_G3_plot31, labels = "AUTO", ncol=3, nrow=5, rel_heights= c(1,1), rel_widths = c(1,1), label_size = 15);InfectRaw_Fig1
```
...
# Monoculture infection_ZEROS REMOVED (no poly): Cell Counts Models, log transformed
...
Singles_no0<-rbind(C1Full_no0,D1Full_no0,F1Full_no0,G3Full_no0)
...
emm<-emmeans(Singles_no0_27and30_mod,  ~ Temp |Day|Symbiont, adjust="Tukey")
cld(emm)
...
emm<-emmeans(Singles_no0_D3D7_mod,  ~ Temp |Day|Symbiont, adjust="Tukey")
cld(emm)
...
C1mod_no0<-lmer(cells.larvae_log~Day*Temp+(1|PotCode), Singles_no0_C1)
anova(C1mod_no0)
#summary(C1mod_no0) #for std err df and t
qqPlot(residuals(C1mod_no0))   #  good

#post hoc diff at each day
 emm<-emmeans(C1mod_no0,  ~ Temp |Day, adjust="Tukey")
cld(emm, Letter="abcdefg")
pairs(emm)
#post hoc within temp
 emm<-emmeans(C1mod_no0,  ~ Day |Temp, adjust="Tukey")
cld(emm, Letter="abcdefg")
pairs(emm)
...
D1mod_no0<-lmer(cells.larvae_log~Day*Temp+(1|PotCode), Singles_no0_D1)
anova(D1mod_no0)
#summary(D1mod) #for std err df and t
qqPlot(residuals(D1mod_no0))   #  low end
  emm<-emmeans(D1mod_no0,  ~ Day |Temp, adjust="Tukey")
cld(emm)
pairs(emm)

emm<-emmeans(D1mod_no0,  ~ Temp |Day, adjust="Tukey")
cld(emm)
pairs(emm) 
```
F1 singles  
...
F1mod_no0<-lmer(cells.larvae_log~Day*Temp+(1|PotCode), Singles_no0_F1)
anova(F1mod_no0)
#summary(F1mod) #for std err df and t
qqPlot(residuals(F1mod_no0))   #  good
  emm<-emmeans(F1mod_no0,  ~ Temp |Day, adjust="Tukey")
cld(emm)
pairs(emm)

emm<-emmeans(F1mod_no0,  ~ Day |Temp, adjust="Tukey")
cld(emm)
pairs(emm)
```
G3 singles  
...
G3mod_no0<-lmer(cells.larvae_log~Day*Temp+(1|PotCode), Singles_no0_G3)
anova(G3mod_no0)
#summary(G3mod) #for std err df and t
qqPlot(residuals(G3mod_no0))   #  good
  emm<-emmeans(G3mod_no0,  ~ Temp|Day, adjust="Tukey")
cld(emm)
pairs(emm)

  emm<-emmeans(G3mod_no0,  ~ Day|Temp, adjust="Tukey")
cld(emm)
pairs(emm)
```
...
Singles_no0_27_mod<-lmer(cells.larvae_log~Day*Symbiont+(1|PotCode), Singles_no0_27)
anova(Singles_no0_27_mod)
#summary(C1mod) #for std err df and t
qqPlot(residuals(Singles_no0_27_mod))   #  good

#post hoc diff at each day
 emm<-emmeans(Singles_no0_27_mod,  ~ Symbiont *Day, adjust="Tukey")
cld(emm)
pairs(emm)
...
Singles_no0_27_mean_plot<-ggplot(data=Singles_no0_27_mean, aes(x=Day.num, y=mean, group = Symbiont, color=Symbiont)) +
  ...
  geom_point(aes(color=Symbiont), size=4, show.legend = T)+  
  geom_line(aes(color=Symbiont),linetype="dotted")+
 scale_color_manual(values = c("#EEA236FF","#46B8DAFF", "#D43F3AFF","#5CB85CFF"))+
                      geom_point(shape = 1,size = 4,colour = "black")+
  xlab("") + #Label the X Axis
       scale_x_continuous(breaks=seq(0, 14, 2), limits =c(0,14))+
       scale_y_log10(breaks = c(0,1,100,1000, 10000), limits=c(.1,5000))+
...
Singles_no0_30_mod<-lmer(cells.larvae_log~Day*Symbiont+(1|PotCode), Singles_no0_30)
anova(Singles_no0_30_mod)
#summary(C1mod) #for std err df and t
qqPlot(residuals(Singles_no0_30_mod))   #  good

#post hoc diff at each day
 emm<-emmeans(Singles_no0_30_mod,  ~ Symbiont *Day, adjust="Tukey")
cld(emm)
pairs(emm)
...
Singles_no0_31_mod<-lmer(cells.larvae_log~Day*Symbiont+(1|PotCode), Singles_no0_31)
anova(Singles_no0_31_mod)
#summary(C1mod) #for std err df and t
qqPlot(residuals(Singles_no0_31_mod))   #  good

#post hoc diff at each day
 emm<-emmeans(Singles_no0_31_mod,  ~ Symbiont *Day, adjust="Tukey")
cld(emm)
pairs(emm)
...
# Monoculture Infection: ZEROS REMOVED (no poly)Plots: by clade,cell counts (count data, with 0s included) 
C1 single infection plot  
```{r}         
C1Full_no0_mean <- ddply(C1Full_no0, c("Day", "Temp"), summarise, #summarize cell counts
                  N    = length(cells.larvae_counts_round[!is.na(cells.larvae_counts_round)]), #calculate the length of the data frame,  excluding NA’s *this part is important*
                 mean = mean(cells.larvae_counts_round, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(cells.larvae_counts_round, na.rm=TRUE), #calculate standard deviation 
                 se   = sd / sqrt(N), #calculate standard error
...
C1Full_no0_mean #display table

#change Day back to Numeric from Factor, reapply the actual numeric dates
C1Full_no0_mean$Day.num<-as.numeric(C1Full_no0_mean$Day)
C1Full_no0_mean$Day.num[C1Full_no0_mean$Day.num == 2] =7
C1Full_no0_mean$Day.num[C1Full_no0_mean$Day.num == 3] =14
C1Full_no0_mean$Day.num[C1Full_no0_mean$Day.num == 1] =3
 
#plot with yaxis log10, real data (not transformed)
C1Full_no0_plot_9000<-ggplot(data=C1Full_no0_mean, aes(x=Day.num, y=mean, group = Temp, color=Temp)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  ...
  ylab(expression(paste("cells/larvae"))) +
  ggtitle("Cladocopium")+
  theme(plot.title = element_text(size=20, face = "italic", hjust=0.5));C1Full_no0_plot_9000
``` 

D1 single infection plot
```{r}     
D1Full_no0_mean <- ddply(D1Full_no0, c("Day", "Temp"), summarise, #summarize cell counts
                 N    = length(cells.larvae_counts_round[!is.na(cells.larvae_counts_round)]), #calculate the length of the data frame,  excluding NA’s *this part is important*
                 mean = mean(cells.larvae_counts_round, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(cells.larvae_counts_round, na.rm=TRUE), #calculate standard deviation 
                 se   = sd / sqrt(N), #calculate standard error
...
D1Full_no0_mean #display table

#change Day back to Numeric from Factor, reapply the actual numeric dates
D1Full_no0_mean$Day.num<-as.numeric(D1Full_no0_mean$Day)
D1Full_no0_mean$Day.num[D1Full_no0_mean$Day.num == 2] =7
D1Full_no0_mean$Day.num[D1Full_no0_mean$Day.num == 3] =14
D1Full_no0_mean$Day.num[D1Full_no0_mean$Day.num == 1] =3

#plot with yaxis log10, real data (not transformed)
D1Full_no0_plot_9000<-ggplot(data=D1Full_no0_mean, aes(x=Day.num, y=mean, group = Temp, color=Temp)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Temp), size=4, show.legend = T)+  
  geom_line(aes(color=Temp),linetype="dotted")+
  ...
  ylab(expression(paste(""))) +
  ggtitle("Durusdinium")+
  theme(plot.title = element_text(size=20, face = "italic", hjust=0.5));D1Full_no0_plot_9000
``` 

F1 single infection  plot
```{r}     
F1Full_no0_mean <- ddply(F1Full_no0, c("Day", "Temp"), summarise, #summarize cell counts
                 N    = length(cells.larvae_counts_round[!is.na(cells.larvae_counts_round)]), #calculate the length of the data frame,  excluding NA’s *this part is important*
                 mean = mean(cells.larvae_counts_round, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(cells.larvae_counts_round, na.rm=TRUE), #calculate standard deviation 
                 se   = sd / sqrt(N), #calculate standard error
...
F1Full_no0_mean #display table

#change Day back to Numeric from Factor, reapply the actual numeric dates
F1Full_no0_mean$Day.num<-as.numeric(F1Full_no0_mean$Day)
F1Full_no0_mean$Day.num[F1Full_no0_mean$Day.num == 2] =7
F1Full_no0_mean$Day.num[F1Full_no0_mean$Day.num == 3] =14
3$Day.num[F1Full_no0_mean$Day.num == 1] =3

#plot with yaxis log10, real data (not transformed)
F1Full_no0_plot_9000<-ggplot(data=F1Full_no0_mean, aes(x=Day.num, y=mean, group = Temp, color=Temp)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
 geom_point(aes(color=Temp), size=4, show.legend = T)+  
  geom_line(aes(color=Temp),linetype="dotted")+
  ...
  ylab(expression(paste(""))) +
  ggtitle("Fugacium")+
  theme(plot.title = element_text(size=20, face = "italic", hjust=0.5));F1Full_no0_plot_9000
``` 

G3 single infection plot
```{r}     
G3Full_no0_mean <- ddply(G3Full_no0, c("Day", "Temp"), summarise, #summarize cell counts
                 N    = length(cells.larvae_counts_round[!is.na(cells.larvae_counts_round)]), #calculate the length of the data frame,  excluding NA’s *this part is important*
                 mean = mean(cells.larvae_counts_round, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(cells.larvae_counts_round, na.rm=TRUE), #calculate standard deviation 
                 se   = sd / sqrt(N), #calculate standard error
...
G3Full_no0_mean #display table 

#change Day back to Numeric from Factor, reapply the actual numeric dates
G3Full_no0_mean$Day.num<-as.numeric(G3Full_no0_mean$Day)
G3Full_no0_mean$Day.num[G3Full_no0_mean$Day.num == 2] =7
G3Full_no0_mean$Day.num[G3Full_no0_mean$Day.num == 3] =14
G3Full_no0_mean$Day.num[G3Full_no0_mean$Day.num == 1] =3

#plot with yaxis log10, real data (not transformed)
G3Full_no0_plot_9000<-ggplot(data=G3Full_no0_mean, aes(x=Day.num, y=mean, group = Temp, color=Temp)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
 geom_point(aes(color=Temp), size=4, show.legend = T)+  
  geom_line(aes(color=Temp),linetype="dotted")+
  ...
  ylab(expression(paste(""))) +
  ggtitle("Gerakladium")+
  theme(plot.title = element_text(size=20, face = "italic", hjust=0.5));G3Full_no0_plot_9000
``` 

Plot all four SINGLE INFECTION together: Figure x
```{r} 
#log scale axis only  
C1Full_no0_plot_9000<-C1Full_no0_plot_9000+ theme(legend.position = "none") #remove legend
D1Full_no0_plot_9000<-D1Full_no0_plot_9000+ theme(legend.position = "none") #remove legend
F1Full_no0_plot_9000<-F1Full_no0_plot_9000+ theme(legend.position = "none") #remove legend
G3Full_no0_plot_9000<-G3Full_no0_plot_9000+ theme(legend.position = "none") #remove legend
grid.arrange(C1Full_no0_plot_9000, D1Full_no0_plot_9000, F1Full_no0_plot_9000,G3Full_no0_plot_9000, nrow = 1) 
```
...
# Monoculture vs poluculture-infection:counts PLOTS (Blue and pink plots) with 0s removed for multi (aka ONLY when they infected with a SPECIFIC zoox)
C1       
C1Full2 = Single cells.larvae raw
ALLcast_9000_cast2a = Alls raw that infected  EDITED per each clade to remove 0s
```{r}      
...
SingleCounts<-SingleCounts[-c(5:9)] #remove metadata
SingleCounts$cells.larvae_round<-SingleCounts$cells.larvae_counts_round #rename
...
CountsCompare2_C1_plot27<-ggplot(data=CountsCompare2_C1_27, aes(x=Day, y=mean, group=single.multi, color=single.multi)) +
  ...
  geom_point(aes(color=single.multi), size=3.5, show.legend = T)+
...
# Monoculture vs polyculture: Pink and Blue Models by symbiont, 0s removed for multi within specific zoox. 
```{r}     
#stats, log10: Normal 
...
CountsCompare2_ALL_mod<-lmer(cells.larvae_log~Temp*Day*single.multi*Symbiont+(1|PotCode),CountsCompare2)
anova(CountsCompare2_ALL_mod)
qqPlot(residuals(CountsCompare2_ALL_mod))   # good
  
emm<-emmeans(CountsCompare2_ALL_mod,  ~ Temp*Day, adjust="Tukey")
cld(emm)
...
CountsCompare2_C1_mod2<-lmer(cells.larvae_round~Temp*Day*single.multi+(1|PotCode),CountsCompare2_C1)
anova(CountsCompare2_C1_mod2)

model.sel(CountsCompare2_C1_mod, CountsCompare2_C1_mod2) #transformed better model
...
qqPlot(residuals(CountsCompare2_D1_mod))   # ok
   emm<-emmeans(CountsCompare2_D1_mod,  ~ single.multi|Temp|Day, adjust="Tukey") 
cld(emm)
pairs(emm)

CountsCompare2_D1_mod2<-lmer(cells.larvae_round~Temp*Day*single.multi+(1|PotCode),CountsCompare2_D1)
anova(CountsCompare2_D1_mod2)

model.sel(CountsCompare2_D1_mod, CountsCompare2_D1_mod2) #transformed better
...
CountsCompare2_F1_mod2<-lmer(cells.larvae_round~Temp*Day*single.multi+(1|PotCode),CountsCompare2_F1)
anova(CountsCompare2_F1_mod2)

model.sel(CountsCompare2_F1_mod, CountsCompare2_F1_mod2)
...
CountsCompare2_G3_mod2<-lmer(cells.larvae_round~Temp*Day*single.multi+(1|PotCode),CountsCompare2_G3)
anova(CountsCompare2_G3_mod2)

model.sel(CountsCompare2_G3_mod, CountsCompare2_G3_mod2)
...
# Monoculture & Polyculture-infection:counts PLOTS (Blue and pink plots)
C1    
C1Full2 = Single cells.larvae raw
...
#check loss of 0s in C1
#CountsCompare_C1counts<-subset(CountsCompare, Symbiont=="C1")
#CountsCompare_C1counts27<-subset(CountsCompare_C1counts, Temp==27)
#CountsCompare_C1counts30<-subset(CountsCompare_C1counts, Temp==30)
#CountsCompare_C1counts31<-subset(CountsCompare_C1counts, Temp==31)
...
CountsCompare_C1_31<-subset(CountsCompare_mean_31, Symbiont=="C1")
 CountsCompare_C1_31<-subset(CountsCompare_C1_31, Day=="3" |Day=="7")
...
CountsCompare_D1_31<-subset(CountsCompare_mean_31, Symbiont=="D1")
 CountsCompare_D1_31<-subset(CountsCompare_D1_31, Day=="3" |Day=="7")
...
CountsCompare_F1_31<-subset(CountsCompare_mean_31, Symbiont=="F1")
 CountsCompare_F1_31<-subset(CountsCompare_F1_31, Day=="3" |Day=="7")
...
CountsCompare_G3_31<-subset(CountsCompare_mean_31, Symbiont=="G3")
 CountsCompare_G3_31<-subset(CountsCompare_G3_31, Day=="3" |Day=="7")
...
POLYCULTURE: 
**Use only Relabundance for within larvae comparisons, use Raw counts for comparision with Single-genus infections **     
...
#write.csv(ALL_RelA_Infect,"ALL_RelA_Infect2_20200814.csv")
...
NumWinner_27RelA_D3a<-NumWinner_27RelA_D3[c("numbClades","Symbiont","cells.larvae")] #keep only cols you need
...
#D14
NumWinner_31RelA_D14<-subset(NumWinner_31RelA, Day==14) #subset day 14
NumWinner_31RelA_D14a<-NumWinner_31RelA_D14[c("numbClades","Symbiont","cells.larvae")] #keep only cols you need
NumWinner_31RelA_D14a_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_31RelA_D14a, mean) #aggregate by symbiont + numbclades
...
# Competative-infection: relative abundance of each clade  
