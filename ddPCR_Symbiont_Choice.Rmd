...
Model 1: lk(cell.counts~Day*Temp) using log transformed! (consider - combinging 'tank' and replicate)
```{r}
#C1
C1mod<-lm(log(cells.larvae+1)~Day*Temp, C1Full)
summary(C1mod)
  qqPlot(residuals(C1mod))
## Day and Day:Temp31 are significant
...
D1mod<-lm(log(cells.larvae+1)~Day*Temp, D1Full)
summary(D1mod)
  qqPlot(residuals(D1mod)) #not amazing but not entirely awful
## Day is significant
  
F1mod<-lm(log(cells.larvae+1)~Day*Temp, F1Full)
summary(F1mod)
  qqPlot(residuals(F1mod)) #not amazing but not entirely awful
# Day:Temp30 is significant
  
G3mod<-lm(log(cells.larvae+1)~Day*Temp, G3Full)
summary(G3mod)
...
```{r}
Allmod<-lmer(log(cells.larvae+1)~Day*Temp*Primer+(1|ALL.number), ALL1)
summary(Allmod)

#not sure if it makes sense to run this like this
Allmod2<-lm(log(cells.larvae+1)~Day*Temp*Primer+ALL.number, ALL1)
summary(Allmod2)
```
