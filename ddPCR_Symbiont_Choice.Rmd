...
# y =0.94x + 0 to convert scope/hemo to ddPCR

```


Right one: logged10 plot 
```{r}
...
Apply hemo.scope conversion and remake df with raw
```{r}
...
#Single Infection or not after 3 days - binary yes or no- INDIVIDUAL INFECTIONS (this is good to go!å)
...
Decide if you can cut 'counttype' out bc you corrected with the regression
```{r}
...
Plot all four SINGLE INFECTION together: good to go
```{r}
#log scale axis only 
...
Single comparision days between symbs, boxplots
```{r} 
#cool plot
...
Heat maps, by clade (temp x time) GOOD
...
#A. Binomial. Did larvae infect or not, by clade and overall  (GOOD)
```{r} 
...
C1 -single alone and single vs multi
```{r}  
...
C1_binary_Multi_plot<-ggplot(data=RelA_binary_C1_mean, aes(x=Day, y=mean, group = Temp, color=Temp)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Temp), size=4, show.legend = T)+ 
  geom_line(aes(color=Temp))+
  ...
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        plot.background =element_blank(), #Set the plot background
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("% infected"))) +
  ggtitle("C1 Multi infection")+
...
D1_binary_Multi_plot<-ggplot(data=RelA_binary_D1_mean, aes(x=Day, y=mean, group = Temp, color=Temp)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Temp), size=4, show.legend = T)+ 
  geom_line(aes(color=Temp))+
...
F1_binary_Multi_plot<-ggplot(data=RelA_binary_F1_mean, aes(x=Day, y=mean, group = Temp, color=Temp)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Temp), size=4, show.legend = T)+ 
  geom_line(aes(color=Temp))+
...
G3_binary_Multi_plot<-ggplot(data=RelA_binary_G3_mean, aes(x=Day, y=mean, group = Temp, color=Temp)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Temp), size=4, show.legend = T)+ 
  geom_line(aes(color=Temp))+
...
I don't think i want to do this. THis needs a lot of work, but not necessarily needed for story, just say with WORDS
```{r}     
#df ALL_RelA
...
Relative abundance Stats, df ALL_RelA_b (good)
```{r}
