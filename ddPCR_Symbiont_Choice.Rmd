...
ALL_27_Day3plot_8000<-ggplot() +  
                  geom_bar(aes(y = cells.larvae, x = ALL.number, fill = Symbiont), data = ALL_27_Day3_8000,
                            stat="identity")+
   ylim(0,3500)+
  ggtitle("Day 3, 27C");ALL_27_Day3plot_8000
```
#Plot infection of individual larvae
27C Day 7
```{r}
...
ALL_27_Day7plot_8000<-ggplot() + 
  geom_bar(aes(y = cells.larvae, x = ALL.number, fill = Symbiont), data = ALL_27_Day7_8000,
                           stat="identity")+
   ylim(0,3500)+
  ggtitle("Day 7, 27C");ALL_27_Day7plot_8000
```
...
ALL_27_Day14plot_8000<-ggplot() + 
  geom_bar(aes(y = cells.larvae, x = ALL.number, fill = Symbiont), data = ALL_27_Day14_8000,
                           stat="identity")+
   ylim(0,3500)+
  ggtitle("Day 14, 27C_8000");ALL_27_Day14plot_8000
```
...
ALL_30_Day3plot_8000<-ggplot() + 
  geom_bar(aes(y = cells.larvae, x = ALL.number, fill = Symbiont), data = ALL_30_Day3_8000,
                           stat="identity")+
   ylim(0,3500)+
  ggtitle("Day 3, 30_8000");ALL_30_Day3plot_8000
```
#Plot infection of individual larvae
30C Day 7
```{r} 
...
ALL_30_Day7plot_8000<-ggplot() + 
  geom_bar(aes(y = cells.larvae, x = ALL.number, fill = Symbiont), data = ALL_30_Day7_8000,
                           stat="identity")+
   ylim(0,3500)+
  ggtitle("Day 7, 30C");ALL_30_Day7plot_8000
```
...
ALL_30_Day14plot_8000<-ggplot() + 
  geom_bar(aes(y = cells.larvae, x = ALL.number, fill = Symbiont), data = ALL_30_Day14_8000,
                           stat="identity")+
   ylim(0,3500)+
  ggtitle("Day 14, 30C");ALL_30_Day14plot_8000
```
...
ALL_31_Day3plot_8000<-ggplot() +  
  geom_bar(aes(y = cells.larvae, x = ALL.number, fill = Symbiont), data = ALL_31_Day3_8000,
                           stat="identity")
 ylim(0,3500)+
  ggtitle("Day 3, 31C_8000");ALL_31_Day3plot_8000
```
##Plot infection of individual larvae
31C Day 7
```{r}
...
ALL_31_Day7plot_8000<-ggplot() + 
  geom_bar(aes(y = cells.larvae, x = ALL.number, fill = Symbiont), data = ALL_31_Day7_8000,
                           stat="identity")+
   ylim(0,3500)+
  ggtitle("Day 14, 31C");ALL_31_Day7plot_8000
```
...
#log transform
C1Full$cells.larvae_log<-log(C1Full$cells.larvae)
  hist(C1Fullt$cells.larvae_log) #more normal
D1Full$cells.larvae_log<-log(D1Full$cells.larvae)
  hist(D1Full$cells.larvae_log) #more normal
F1Full$cells.larvae_log<-log(F1Full$cells.larvae)
  hist(F1Full$cells.larvae_log) #more normal
G3Full$cells.larvae_log<-log(G3Full$cells.larvae)
  hist(G3Full$cells.larvae_log) #more normal, scewed right
```

Model 1: lk(cell.counts~Day*Temp) using log transformed!
```{r}
#C1
#C1mod<-lmer(log(cells.larvae+1)~Day*Temp+(1|Pot), C1Full) <- when I add "Pot" as random effect, it gives me singularity error

## ANOVA vs lm()?
C1mod<-aov(log(cells.larvae+1)~Day*Temp, C1Full)
summary(C1mod)
  qqPlot(residuals(C1mod))
#Significant: Day, Temp, Day:Temp
...
D1mod<-aov(log(cells.larvae+1)~Day*Temp, D1Full)
summary(D1mod)
  qqPlot(residuals(D1mod)) #not amazing but not entirely awful
## Significant: Day

F1mod<-aov(log(cells.larvae+1)~Day*Temp, F1Full)
summary(F1mod)
  qqPlot(residuals(F1mod)) #not amazing but not entirely awful
##Significance: Day:Temp
TukeyHSD(F1mod)
  G3mod<-aov(log(cells.larvae+1)~Day*Temp, G3Full)
summary(G3mod)
...
```{r}
Allmod<-lmer(log(cells.larvae+1)~Day*Temp*Primer+(1|ALL.number), ALL1)  #All.number is the larvae ID
summary(Allmod)
#conduct post hoc test
emm_Allmod = emmeans(Allmod, ~ Day * Temp * Primer, adjust="tukey")
  #letter display
cld(emm_Allmod, Letters=c(LETTERS))
#day is coming out "5.96" figure out why
```

Figure out: how to run the mean of the preicted infection (single infections) against the actual infection in the multi infection.
