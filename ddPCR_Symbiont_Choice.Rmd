...
library("qqplotr")
library("blmeco") #this isn't loading
...
library(lmerTest)
library(ggsignif)
library(HH) #not loading
library(PMCMR)
library(tidyverse)
library(arm) #not loading
library(jtools)
library(broom)
library(ggstance)
library(officer)
...
Countdat1<-Countdat[!(Countdat$Symbiont=="CONTROL"),] #remove controls
...
#cast by primers - do this to see which larvae not all primers amplified and cut them for Relative Abund df
 ALLcast_9000_cast<-dcast(ALL_9000a, ALL.number+Day+Temp+Pot~Primer, value.var = "cells.larvae") # cast()
 

### For Relative Abundance df, REMOVE larvae without amplification in all 4 symbiont genera: 
 
ALL_9000b<-ALLcast_9000_cast
ALL_9000b<-na.omit(ALL_9000b) #remove larva with missing data
```
...
```{r}     
# Raw df for comparision of counts within INDIVIDUAL GENERA between Single vs Multi
...
#remove the single larva at D14 31C due to low n in treatment, ID 115
ALLcast_9000_cast2a<-ALLcast_9000_cast2a[!(ALLcast_9000_cast2a$ALL.number=="115"),]

 # Relative Abundance df to use to compare infection by clade in multi. 
ALLcast_9000_cast2b<-ALL_9000b #make a copy
...
#remove the single larva at D14 31C due to low n, ID 115
ALLcast_9000_cast2b<-ALLcast_9000_cast2b[!(ALLcast_9000_cast2b$ALL.number=="115"),]
...
Organize dfs ddPCR :
```{r}       
subset(Polycountdat, select = -c(Replicate) ) #remove rep col (na for poly)


#C1   
#organize the ddPCR data for individual infections  for MONOCULTURE
C1_vars<-c("Temp","Day", "Info","Pot","cells.larvae")
...
C1countdat<-subset(C1countdat, select = -c(Replicate) ) #remove replicate
...
D1_vars<-c("Temp","Day", "Info","Pot","cells.larvae")
...
D1countdat<-subset(D1countdat, select = -c(Replicate) ) #remove replicate
...
#F1
#organize the ddPCR data   
F1_vars<-c("Temp","Day", "Info","Pot","cells.larvae")
...
F1countdat<-subset(F1countdat, select = -c(Replicate) ) #remove replicate
...
#G3
#organize the ddPCR data 
G3_vars<-c("Temp","Day", "Info","Pot","cells.larvae")
...
G3countdat<-subset(G3countdat, select = -c(Replicate) ) #remove replicate
...
#ALL 
#ddpcr
Polycast_9000_cast2b_PolyTotal<-ALLcast_9000_cast2b #make df copy
Polycast_9000_cast2b_PolyTotal$Info<-"Poly"          # make info column
colnms=c("C1", "D1", "F1", "G3")
Polycast_9000_cast2b_PolyTotal$cells.larvae<-rowSums(Polycast_9000_cast2b_PolyTotal[,colnms]) #make new col with sum of all genera
Poly_vars<-c("Temp","Day", "Info","Pot","cells.larvae")  #select cols
Poly_9000b<-Polycast_9000_cast2b_PolyTotal[Poly_vars]
colnames(Poly_9000b)[colnames(Poly_9000b)=="Info"] <- "Symbiont"
Poly_9000b$count.type<-"ddPCR"
hist(Poly_9000b$cells.larvae)

#organize hemo data
Polycountdat<-subset(Countdat_2, Symbiont=="ALL")
Polycountdat<- subset(Polycountdat, select = -c(Replicate) ) #remove rep col (na for poly)
PolyFull<-rbind(Poly_9000b,Polycountdat) #combine hemo and ddPCR
hist(PolyFull$cells.larvae) 
PolyFull$cells.larvae_log<-log10(PolyFull$cells.larvae+1)
hist(PolyFull$cells.larvae_log) #normal
...
#write.csv(scopehemoFull_mean,"scopehemoFull_mean20200820.csv")

scopehemoFull_meanVars<-c("Temp","Day","Pot", "Symbiont","mean","count.type","N") #keep only cols you need
...
scopehemoFull_mean2<-subset(scopehemoFull_mean2, N=="3")   #remove means groups without n=3
scopehemoFull_mean2$Symbiont <- gsub('ALL', 'Poly', scopehemoFull_mean2$Symbiont) #change "all" to "Poly"
scopehemoFull_mean2$Day<-as.factor(as.character(scopehemoFull_mean2$Day))
scopehemoFull_mean2$Pot<-as.factor(as.character(scopehemoFull_mean2$Pot))
scopehemoFull_mean2$Symbiont<-as.factor(as.character(scopehemoFull_mean2$Symbiont))
scopehemoFull_mean2$count.type<-as.factor(as.character(scopehemoFull_mean2$count.type))



# raw means for ddPCR  only
ddPCRcompare<-rbind(C1_9000b,D1_9000b,F1_9000b,G3_9000b,Poly_9000b)
...
ddPCRcompareVars<-c("Temp","Day","Pot", "Symbiont","mean","count.type","N")
...
#remove data groups where n<3
ddPCRcompare_mean2<-subset(ddPCRcompare_mean2, N>="3") #subset =to or greater than

 # combine Scope and ddPCR into one df
...
CountsMOD4<-lm(ddPCRmean_log ~ 0 + Countmean_log*count.type.x,data = scopehemo_ddPCRCompare)
anova(CountsMOD4)
...
#write.csv(scopehemo_ddPCRCompare,"scopehemo_ddPCRCompare20200820.csv")
#scopehemo_ddPCRCompare_all<-read.csv("scopehemo_ddPCRCompare1.csv")
...
lmmodel2_plot<-lmodel2(ddPCRmean_log~Countmean_log+Countmean_log:count.type.x, data = scopehemo_ddPCRCompare)  
plot(lmmodel2_plot)
plot(lmmodel2_plot, "MA")
...
#add in lmodel2 slope/intercept
  ...
  #geom_smooth(aes(ymin=.73, ymax=1.20, fill =count.type.x, colour=count.type.x,stat="identity"))
   ...
   geom_abline(intercept = 0, slope = 1.04, color = "red", linetype = "dashed")
```
...
C1Fullscopehemo$cells.larvae_log<-(C1Fullscopehemo$cells.larvae_log*1.04) #convert and check
#  plot(C1Fullscopehemo$cells.larvae_log, C1Fullscopehemo$cells.larvae_log) #check
C1Full<-rbind(C1FullddPCR,C1Fullscopehemo) #rebind with ddPCR
C1Full$cells.larvae_counts_unlog<-(10^C1Full$cells.larvae_log)-1  #converted all log back to Raw counts WHY
#C1Full$cells.larvae_counts_round<-round(C1Full$cells.larvae_counts)


#plot(C1Full$cells.larvae_log, C1Full$cells.larvae_log2) #check
...
D1Fullscopehemo$cells.larvae_log<-(D1Fullscopehemo$cells.larvae_log*1.04) #convert and check
...
F1Fullscopehemo$cells.larvae_log<-(F1Fullscopehemo$cells.larvae_log*1.04) #convert and check
...
G3Fullscopehemo$cells.larvae_log<-(G3Fullscopehemo$cells.larvae_log*1.04) #convert and check
...
#ALL
ALLFullddPCR<-subset(ALLFull, count.type=="ddPCR")
ALLFullscopehemo<-subset(ALLFull, count.type=="hemo"|count.type=="scope")
#ALLFullscopehemo$cells.larvae_log_Pre<-ALLFullscopehemo$cells.larvae_log #save old to check then comment out
ALLFullscopehemo$cells.larvae_log<-(ALLFullscopehemo$cells.larvae_log*1.04) #convert and check
ALLFull<-rbind(ALLFullddPCR,ALLFullscopehemo) #rebind with ddPCR
ALLFull$cells.larvae_counts_unlog<-(10^ALLFull$cells.larvae_log)-1  #converted all log back to Raw counts
#ALLFull$cells.larvae_counts_round<-round(ALLFull$cells.larvae_counts)

#plot(ALLFull$cells.larvae, ALLFull$cells.larvae_counts_round) #check

Monoculture<-rbind(C1Full,D1Full, F1Full, G3Full)
MonoPoly<-rbind(C1Full,D1Full, F1Full, G3Full,ALLFull)
...
# Larval Mortality
```{r}       
Mort.df<-read.csv("SC_mortality_20200722.csv") #read mortality df
  hist(Mort.df$Total)
Mort.df$Total_log<-log10(Mort.df$Total+1) #transform log10
  hist(Mort.df$Total_log) #use log
Mort.df$Total_sqrt<-sqrt(Mort.df$Total) #transform log10
  hist(Mort.df$Total_sqrt) #

  
Mort.df$Pot<-as.factor(as.character(Mort.df$Pot))
  Mort.df$Temp<-as.factor(as.character(Mort.df$Temp))
 Mort.df$Day<-as.factor(as.character(Mort.df$Day))
 
  Mort.df_mean<- ddply(Mort.df, c( "Temp","Symbiont","Day"), summarise, #summarize cell counts
                 N    = length(Total_log[!is.na(Total_log)]), #calculate the length of the data frame, excluding NA’s *this part is important*
                 mean = mean(Total_log, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(Total_log, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
                 max = max(Total_log, na.rm=TRUE) #calculate max, could also calculate min () if desired
);Mort.df_mean #display table

 
 Mort.df_mod<-lmer(Total_sqrt~Temp*Symbiont*Day+(1|Pot), Mort.df)
anova(Mort.df_mod)
qqPlot(residuals(Mort.df_mod))   #  meh 
#post hoc
 emm<-emmeans(Mort.df_mod,  ~ Symbiont*Temp, adjust="Tukey")
...
#### AH binomial mixed effects model
...
#attach(Mort.df_NEW_D7)
#x7<-cbind(Alive, Failures) 
...
#Random effects: Pot  
#`stressFINALmod<-glmer(x~temp*symbiont*day + (1|pot), family=binomial)`
...
cld(emm, Letter="abcdefg")
pairs(emm)
#dispersion_glmer(Mort.df_NEW_D7_mod) #The package wont install :(

detach(Mort.df_NEW)


#model of mono vs poly with symbiont as nested

Mort.df_MvP_NEW<-Mort.df_NEW                                                 #make a copy
Mort.df_MvP_NEW$Symbiont<-as.character(Mort.df_MvP_NEW$Symbiont)             # change to character
Mort.df_MvP_NEW$MonoPoly<-Mort.df_MvP_NEW$Symbiont                           #duplicate
Mort.df_MvP_NEW$MonoPoly[Mort.df_MvP_NEW$MonoPoly == "C1" ] <- "Mono"        #change monos to mono
Mort.df_MvP_NEW$MonoPoly[Mort.df_MvP_NEW$MonoPoly == "D1" ] <- "Mono"
Mort.df_MvP_NEW$MonoPoly[Mort.df_MvP_NEW$MonoPoly == "F1" ] <- "Mono"
Mort.df_MvP_NEW$MonoPoly[Mort.df_MvP_NEW$MonoPoly == "G3" ] <- "Mono"
Mort.df_MvP_NEW$MonoPoly[Mort.df_MvP_NEW$MonoPoly == "All" ] <- "Poly"
Mort.df_MvP_NEW$MonoPoly<-as.factor(Mort.df_MvP_NEW$MonoPoly)
Mort.df_MvP_NEW<-subset(Mort.df_MvP_NEW, MonoPoly=="Mono"|MonoPoly=="Poly") #remove controls 

attach(Mort.df_MvP_NEW)
x<-cbind(Alive, Failures) 

Mort.df_NEW_MvP_mod<-glmer(x~MonoPoly*Temp*Day + (1|Pot), family=binomial) #why isn't this running? I think it's in the "y"?
summary(Mort.df_NEW_MvP_mod)
Anova(Mort.df_NEW_MvP_mod)

detach(Mort.df_MvP_NEW)


#New plot by temp (20210104)
...
# I think you can delete all below#############


 #try only plotting % decrease aka total mortality by pot
 mort_test<-subset(Mort.df, Day==1|Day==14)
 #write.csv(mort_test, "mort_test.csv")
 Mord.df2<-read.csv("mort_test.csv")
 
 Mord.df2_mean<- ddply(Mord.df2, c( "Temp","Symbiont"), summarise, #summarize cell counts
                 N    = length(PercentSurv[!is.na(PercentSurv)]), #calculate the length of the data frame, excluding NA’s *this part is important*
                 mean = mean(PercentSurv, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(PercentSurv, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
                 max = max(PercentSurv, na.rm=TRUE) #calculate max, could also calculate min () if desired
);Mord.df2_mean #display table

Mord.df2_mean_27<-subset(Mord.df2_mean, Temp=="27")
Mord.df2_mean_30<-subset(Mord.df2_mean, Temp=="30")
Mord.df2_mean_31<-subset(Mord.df2_mean, Temp=="31")


#mort mean by temp
 Mord.df2_temp_mean<- ddply(Mord.df2, c( "Temp"), summarise, #summarize cell counts
                 N    = length(PercentSurv[!is.na(PercentSurv)]), #calculate the length of the data frame, excluding NA’s *this part is important*
                 mean = mean(PercentSurv, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(PercentSurv, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
                 max = max(PercentSurv, na.rm=TRUE) #calculate max, could also calculate min () if desired
);Mord.df2_temp_mean #display table
 ...
 #test stats for this:
 hist(Mord.df2$PercentSurv)
Mord.df2$PercentSurv_log<-log10(Mord.df2$PercentSurv+1)
hist(Mord.df2$PercentSurv_log)

#Mord.df2$PercentSurv_sqrt<-sqrt(Mord.df2$PercentSurv+1) #not better
#hist(Mord.df2$PercentSurv_sqrt)

Mord.df2$Temp<-as.factor(as.character(Mord.df2$Temp))
Mord.df2$Pot<-as.factor(as.character(Mord.df2$Pot))

Mort.df2_mod<-lmer(PercentSurv_log~Temp*Symbiont+(1|Pot), Mord.df2)
anova(Mort.df2_mod)
qqPlot(residuals(Mort.df2_mod))   #  meh 
#post hoc
 emm<-emmeans(Mort.df2_mod,  ~ Symbiont*Temp, adjust="Tukey")
cld(emm, Letter="abcdefg")
pairs(emm)

 
Mord.df2_mean$Temp<-as.factor(as.character(Mord.df2_mean$Temp))
Mord.df2_mean$Symbiont <- factor(Mord.df2_mean$Symbiont, levels = c("C1", "D1", "F1","G3","All","Control"))

Mort_plot2<-ggplot(data=Mord.df2_mean, aes(x=Temp, y=mean, group = Symbiont, colour=Symbiont)) +
scale_colour_manual(values=c( "#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF","black","grey"))+
geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0.3, position=position_dodge(0.3))+
  geom_point(aes(group=Symbiont), size = 4,position=position_dodge(0.3))+
     geom_point(shape = 1,size = 4,colour = "black",position=position_dodge(0.3))+
xlab("Temp") + #Label the X Axis 
 # scale_x_discrete(limits=c("27","30","31"))+
...
ylab(expression(paste("% Surviorship"))) +
ggtitle("Percent survivorship at day 14")+
theme(plot.title = element_text(size=20));Mort_plot2


 
 #### DELETE
 #lets try GLMER poisson
Mort.df_modGLMER1<-glmer(Total ~ Temp*Symbiont*Day+(1|Pot), data = Mort.df, family = poisson(link = "log"))
Anova(Mort.df_modGLMER1)
qqPlot(residuals(Mort.df_modGLMER1))   #  meh 

Mort.df_27_modGLMER2<-glmer(Total ~ Symbiont*Day+(1|Pot), data = Mort.df_27, family = poisson(link = "identity"))
Anova(Mort.df_27_modGLMER2)
qqPlot(residuals(Mort.df_27_modGLMER2))   #  meh 


#Stats
Mort.df_27<-subset(Mort.df, Temp==27) #subset by temp
Mort.df_30<-subset(Mort.df, Temp==30)
Mort.df_31<-subset(Mort.df, Temp==31)

#27
#models 
hist(Mort.df_27$Total_log)
Mort.df_27_mod<-lmer(Total_log~Day*Symbiont+(1|Pot), Mort.df_27)
anova(Mort.df_27_mod)
qqPlot(residuals(Mort.df_27_mod))   #  meh 
#post hoc diff at each day
 emm<-emmeans(Mort.df_27_mod,  ~ Symbiont|Day, adjust="Tukey")
cld(emm, Letter="abcdefg")
pairs(emm)

#lets try GLMER poisson
Mort.df_27_modGLMER1<-glmer(Total ~ Symbiont*Day+(1|Pot), data = Mort.df_27, family = poisson(link = "log"))
Anova(Mort.df_27_modGLMER1)
qqPlot(residuals(Mort.df_27_modGLMER1))   #  meh 
...
hist(Mort.df_30$Total_log) #not great

Mort.df_30_mod<-lmer(Total_log~Day*Symbiont+(1|Pot), Mort.df_30)
anova(Mort.df_30_mod)
qqPlot(residuals(Mort.df_30_mod))   #  this is awful 
#post hoc diff at each day
 emm<-emmeans(Mort.df_30_mod,  ~ Symbiont|Day, adjust="Tukey")
cld(emm, Letter="abcdefg")
pairs(emm)

Mort.df_30_modGLMER1<-glmer(Total ~ Symbiont*Day+(1|Pot), data = Mort.df_30, family = poisson(link="log"))
Anova(Mort.df_30_modGLMER1)
qqPlot(residuals(Mort.df_30_modGLMER1))   #  meh 

Mort.df_30_modGLMER2<-glmer(Total ~ Symbiont*Day+(1|Pot), data = Mort.df_30, family = poisson(link="identity"))
Anova(Mort.df_30_modGLMER2)
qqPlot(residuals(Mort.df_30_modGLMER2))   #  meh 

#31
hist(Mort.df_31$Total_log) #not great


hist(Mort.df_31$Total_log) #not great

Mort.df_31_mod<-lmer(Total_log~Day*Symbiont+(1|Pot), Mort.df_31)
anova(Mort.df_31_mod)
qqPlot(residuals(Mort.df_31_mod))   #  this is awful 
#post hoc diff at each day
 emm<-emmeans(Mort.df_31_mod,  ~ Symbiont|Day, adjust="Tukey")
cld(emm, Letter="abcdefg")
pairs(emm)

Mort.df_31_modGLMER1<-glmer(Total ~ Symbiont*Day+(1|Pot), data = Mort.df_31, family = poisson(link="log"))
Anova(Mort.df_31_modGLMER1)
qqPlot(residuals(Mort.df_31_modGLMER1))   #  meh 

Mort.df_31_modGLMER2<-glmer(Total ~ Symbiont*Day+(1|Pot), data = Mort.df_31, family = poisson(link="identity"))
Anova(Mort.df_31_modGLMER2)
qqPlot(residuals(Mort.df_31_modGLMER2))   #  meh 
...
#PLOTS
# means (not transformed)
Mort.mean<- ddply(Mort.df, c("Day", "Temp","Symbiont"), summarise, #summarize cell counts
                 N    = length(Total[!is.na(Total)]), #calculate the length of the data frame, excluding NA’s *this part is important*
                 mean = mean(Total, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(Total, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
                 max = max(Total, na.rm=TRUE) #calculate max, could also calculate min () if desired
);Mort.mean #display table

Mort.mean$Day<-as.numeric(Mort.mean$Day)

Mort.mean_27<-subset(Mort.mean, Temp=="27")
Mort.mean_30<-subset(Mort.mean, Temp=="30")
Mort.mean_31<-subset(Mort.mean, Temp=="31")



#27
 Mort27_plot<-ggplot(data=Mort.mean_27, aes(x=Day, y=mean, group = Symbiont, color=Symbiont)) +
  ...
  ggtitle("27C Mortality")+
  theme(plot.title = element_text(size=20));Mort27_plot
 
 #30
 Mort30_plot<-ggplot(data=Mort.mean_30, aes(x=Day, y=mean, group = Symbiont, color=Symbiont)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point( size=4, show.legend = T)+ 
      scale_color_manual(values=c("black", "#D43F3AFF","grey","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
  ...
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("# Larvae"))) +
  ggtitle("30C Mortality")+
  theme(plot.title = element_text(size=20));Mort30_plot
 
 #31
 Mort31_plot<-ggplot(data=Mort.mean_31, aes(x=Day, y=mean, group = Symbiont, color=Symbiont)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point( size=4, show.legend = T)+ 
      scale_color_manual(values=c("black", "#D43F3AFF","grey","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
  ...
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("# Larvae"))) +
  ggtitle("31C Mortality")+
  theme(plot.title = element_text(size=20));Mort31_plot

 
Mort27_plot<-Mort27_plot+ theme(legend.position = "none") #remove legend
Mort30_plot<-Mort30_plot+ theme(legend.position = "none") #remove legend
#Mort31_plot<-Mort31_plot+ theme(legend.position = "none") #remove legend
grid.arrange(Mort27_plot, Mort30_plot, Mort31_plot, nrow = 1) 


#redo with Day 1 gone
Mort.mean_27_noD1<-subset(Mort.mean_27, Day=="7"|Day=="14")
Mort.mean_30_noD1<-subset(Mort.mean_30, Day=="7"|Day=="14")
Mort.mean_31_noD1<-subset(Mort.mean_31, Day=="7"|Day=="14")

#27
 Mort.mean_27_noD1_plot<-ggplot(data=Mort.mean_27_noD1, aes(x=Day, y=mean, group = Symbiont, color=Symbiont)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point( size=4, show.legend = T)+ 
      scale_color_manual(values=c("black", "#D43F3AFF","grey","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
  ...
  ylab(expression(paste("# Larvae"))) +
  ggtitle("27C Mortality")+
  theme(plot.title = element_text(size=20));Mort.mean_27_noD1_plot
 
 #30
 Mort.mean_30_noD1_plot<-ggplot(data=Mort.mean_30_noD1, aes(x=Day, y=mean, group = Symbiont, color=Symbiont)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point( size=4, show.legend = T)+ 
      scale_color_manual(values=c("black", "#D43F3AFF","grey","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
  ...
  ylab(expression(paste("# Larvae"))) +
  ggtitle("30C Mortality")+
  theme(plot.title = element_text(size=20));Mort.mean_30_noD1_plot
 
 #31
 Mort.mean_31_noD1_plot<-ggplot(data=Mort.mean_31_noD1, aes(x=Day, y=mean, group = Symbiont, color=Symbiont)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point( size=4, show.legend = T)+ 
      scale_color_manual(values=c("black", "#D43F3AFF","grey","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
  ...
  ylab(expression(paste("# Larvae"))) +
  ggtitle("31C Mortality")+
  theme(plot.title = element_text(size=20));Mort.mean_31_noD1_plot

 
Mort.mean_27_noD1_plot<-Mort.mean_27_noD1_plot+ theme(legend.position = "none") #remove legend
Mort.mean_30_noD1_plot<-Mort.mean_30_noD1_plot+ theme(legend.position = "none") #remove legend
Mort.mean_31_noD1_plot<-Mort.mean_31_noD1_plot+ theme(legend.position = "none") #remove legend
grid.arrange(Mort.mean_27_noD1_plot, Mort.mean_30_noD1_plot, Mort.mean_31_noD1_plot, nrow = 1) 
...
DID LARVAE INFECT OR NOT (binomial)

BINOMIAL DID IT INFECT
BINOMIAL



# Monoculture: Binomial: Did a larvae infect (Y/N) modeles and plots
...
FIRST make df for ALLs with proper binomial
ORganize df: ddPCR and then combine with monoculture ALLs
```{r}     
#ALLcast_9000_cast2a includes all samples even if not all clades amped
...
#ALL_RelAb<-dcast(ALL_RelAb, ALL.number+Temp+Day+Pot~Primer, value.var = "cell.larvae_round") # cast()
...
#write.csv(RelA_binaryb,"RelA_binaryb.csv")
...
#remove the single row with only 0s and NAs to be conservative
...
# RelA_binaryb$PotCode<-paste0(RelA_binaryb$Temp, RelA_binaryb$Pot)
# RelA_binaryb$PotCode<-as.factor(RelA_binaryb$PotCode) #make factor
...
AllCountvars<-c("Temp","Day","cells.larvae","Pot","count.type") #hemoscope prep
ALLFullscopehemo2<-ALLFullscopehemo[AllCountvars]
ALLFullscopehemo2$Infect[ALLFullscopehemo2$cells.larvae>0]<-1  
ALLFullscopehemo2$Infect[ALLFullscopehemo2$cells.larvae==0]<-0
ALLFullscopehemo3<-subset(ALLFullscopehemo2, select = -c(cells.larvae))# drop cells.larvae col

All_binomial<-rbind(ALLFullscopehemo3,RelA_binaryb_2) #combine for the ALL BINOMIAL ONLY DF
All_binomial$Symbiont<-"All" #create symbiont col

All_binomial$Infect<-as.factor(All_binomial$Infect)
  All_binomial$count.type<-as.factor(All_binomial$count.type)
  

#make DF for All cell densities for COUNTS (not binomial)
...
ALLcast_9000_cast3<-ALLcast_9000_cast2c[ALLcast_9000_cast2cAvars] # new df
  ...
  ALLFullscopehemo4<-subset(ALLFullscopehemo2, select = -c(Infect)) #prep scope hemo df
  
ALL_forCounts<-rbind(ALLFullscopehemo4,ALLcast_9000_cast3)
ALL_forCounts<-na.omit(ALL_forCounts) 
```
...
BinaryInfect<-rbind(BinaryInfect, All_binomial) #add the ALLLS for JUST this assay in here (calc above)
...
#Full model including all clades/temp/time:
 Binary_mod<-glmer(Infect~Symbiont*Temp*Day+(1|PotCode), family="binomial", BinaryInfect) #
  Anova(Binary_mod)
dispersion_glmer(Binary_mod) # no overdispersion  .77
  emm<-emmeans(Binary_mod,  ~ Symbiont*Temp*Day)
  pairs(emm)
  cld(emm) 
  
    emm<-emmeans(Binary_mod,  ~ Temp*Day|Symbiont)
cld(emm)
  
#C1 only
BinaryInfect_C1<-subset(BinaryInfect, Symbiont=="C1")
 Binary_mod_C1<-glmer(Infect~Temp*Day+(1|PotCode), family="binomial", BinaryInfect_C1) #
  Anova(Binary_mod_C1)
dispersion_glmer(Binary_mod_C1) # no overdispersion  .77
  emm<-emmeans(Binary_mod_C1,  ~ Day|Temp)
  pairs(emm)
  cld(emm) 
...
dispersion_glmer(BinaryInfect_27and30_mod) # .75
  emm<-emmeans(BinaryInfect_27and30_mod,  ~ Symbiont*Day|Temp)
  pairs(emm)
  cld(emm)
emm<-emmeans(BinaryInfect_27and30_mod,  ~ Symbiont*Day*Temp)
  pairs(emm)
...
dispersion_glmer(BinaryInfect_D3andD7_mod) # no overdispersion  1.01
  emm<-emmeans(BinaryInfect_D3andD7_mod,  ~ Symbiont*Temp*Day)
  pairs(emm)
  cld(emm) 
```
...
Binary_31_mean_plot<-Binary_31_mean_plot+ theme(legend.position = "none") #remove legend
grid.arrange(Binary_27_mean_plot, Binary_30_mean_plot, Binary_31_mean_plot, nrow = 1)
```


# Monoculture vs Polyculture by clade: Binomial TAKE 2 -PINK and BLUE binomial: 20200824  
all amplifications= ALLcast_9000_cast2a (includes NAs)
Plots and comparison models singe v multi 
  
Did larvae with single clades infect more or less than larvae offered multiple clades: set up dfs
```{r}   
#stats    
#df for multi:RelA_binaryb, df for Individuals:  BinaryInfect

# Set up to run models by GENUS ddPCR
# ALL - C1
RelA_binarybb_C1<-RelA_binaryb[,c("Temp","Day","PotCode","C1", "Symbiont")] #keep only cols you need
...
RelA_binarybb_D1<-RelA_binaryb[,c("Temp","Day","PotCode","D1","Symbiont")] #keep only cols you need
...
RelA_binarybb_F1<-RelA_binaryb[,c("Temp","Day","PotCode","F1","Symbiont")] #keep only cols you need
...
RelA_binarybb_G3<-RelA_binaryb[,c("Temp","Day","PotCode","G3","Symbiont")] #keep only cols you need
...
#Individual by clade (monoculture df binary already, new col for infect type)
binaryb_C1_binaryb<-Binary_C1[,c("Temp","Day","PotCode","Symbiont","Infect")]
binaryb_C1_binaryb$infectType<-"Single"
binaryb_D1_binaryb<-Binary_D1[,c("Temp","Day","PotCode","Symbiont","Infect")]
binaryb_D1_binaryb$infectType<-"Single"
binaryb_F1_binaryb<-Binary_F1[,c("Temp","Day","PotCode","Symbiont","Infect")]
binaryb_F1_binaryb$infectType<-"Single"
binaryb_G3_binaryb<-Binary_G3[,c("Temp","Day","PotCode","Symbiont","Infect")]
binaryb_G3_binaryb$infectType<-"Single"


#Individual Infections of MULTI larvae: only use this data to compare to SINGLE infections. PotCode is coded to keep separate single and multi pots
...
binarybInfect_Fig<-plot_grid(C1_binaryb_SinMul_plot_27, C1_binaryb_SinMul_plot_30, C1_binaryb_SinMul_plot_31,D1_binaryb_SinMul_plot_27, D1_binaryb_SinMul_plot_30, D1_binaryb_SinMul_plot_31,F1_binaryb_SinMul_plot_27, F1_binaryb_SinMul_plot_30, F1_binaryb_SinMul_plot_31,G3_binaryb_SinMul_plot_27, G3_binaryb_SinMul_plot_30, G3_binaryb_SinMul_plot_31, labels = "AUTO", ncol=3, nrow=4, rel_heights= c(1,1), rel_widths = c(1,1), label_size = 15);binarybInfect_Fig 
...
#add to plot
binarybInfect_Figb<-grid.arrange(arrangeGrob(binarybInfect_Fig, left = y.grob, bottom = x.grob))
#save
#ggsave(filename="Figures/binarybInfect_Fig20200816.pdf", plot=binarybInfect_Fig, dpi=300, width=12, height=17, units="in")
``` 
...
# Monoculture and poly infection: Cell Counts Models and Plots (updated 20200822)
Model: `lmer(Infect~Symbiont*Temp*Day+(1|PotCode))` 
Single infections (histograms and full model) 
```{r}            
 
#set up the All df for this specific model! use only poly larva with ddPCR for all genera/larva +hemoscope
ALL_forCounts$cells.larvae_log<-log10(ALL_forCounts$cells.larvae+1) #log alls
ALL_forCounts$Symbiont<-"All" #add symbiont col
...
ALLFull_no0<-subset(ALL_forCounts, cells.larvae>0)
  ALLFull_no0$Symbiont<-as.factor(ALLFull_no0$Symbiont)
  ALLFull_no0$count.type<-as.factor(ALLFull_no0$count.type)
...
hist(ALLFull_no0$cells.larvae_log) # normal

Singles_Mono<-rbind(C1Full_no0,D1Full_no0,F1Full_no0,G3Full_no0) #combine mono df
Singles_Mono<-subset(Singles_Mono, select = -c(Replicate, cells.larvae) ) #get rid of original cells.larvae, keep new counts
names(Singles_Mono)[names(Singles_Mono)=="cells.larvae_counts_unlog"] <- "cells.larvae" #rename the new counts cells.larvae for consistency
Singles_Mono$count.type<-as.factor(Singles_Mono$count.type)

Singles_no0<-rbind(Singles_Mono,ALLFull_no0)  ####THis is the df for Cell Densities! 20200822
hist(Singles_no0$cells.larvae_log) # normal 

#rename Pot interaction as a code: Pot_Temp
 Singles_no0$PotCode<-paste0(Singles_no0$Temp, Singles_no0$Pot,Singles_no0$Symbiont )
 Singles_no0$PotCode<-as.factor(Singles_no0$PotCode) #make factor

 
 
 
 
###this still contains data for day 14 for C, All, F, and G. You prob want to remove that for MODELS. you try this below in next section:  
 
#Full model  
## lmer  symbionts*days*temp
Single_no0_all_mod<-lmer(cells.larvae_log~Symbiont*Day*Temp+(1|PotCode), Singles_no0)
anova(Single_no0_all_mod) 
qqPlot(residuals(Single_no0_all_mod))   # good

  emm<-emmeans(Single_no0_all_mod,  ~ Symbiont|Temp *Day, adjust="Tukey")
cld(emm)
pairs(emm)

emm<-emmeans(Single_no0_all_mod,  ~ Temp*Day|Symbiont , adjust="Tukey")
cld(emm)
pairs(emm)

emm<-emmeans(Single_no0_all_mod,  ~ Symbiont*Day|Temp , adjust="Tukey")
cld(emm)
pairs(emm)

#27C and 30C ONLY
Singles_no0_27and30<-subset(Singles_no0, Temp=="27"|Temp=="30")

Singles_no0_27and30_mod<-lmer(cells.larvae_log~Symbiont*Day*Temp+(1|PotCode), Singles_no0_27and30)
anova(Singles_no0_27and30_mod) 
qqPlot(residuals(Singles_no0_27and30_mod))   # good

  emm<-emmeans(Singles_no0_27and30_mod,  ~ Symbiont*Temp *Day, adjust="Tukey")
cld(emm)

emm<-emmeans(Singles_no0_27and30_mod,  ~ Symbiont*Day|Temp, adjust="Tukey")
cld(emm)
...
#D3D7 ONLY
Singles_no0_D3D7<-subset(Singles_no0, Day=="3"|Day=="7")

Singles_no0_D3D7_mod<-lmer(cells.larvae_log~Symbiont*Day*Temp+(1|PotCode), Singles_no0_D3D7)
anova(Singles_no0_D3D7_mod) 
qqPlot(residuals(Singles_no0_D3D7_mod))   # good

  emm<-emmeans(Singles_no0_D3D7_mod,  ~ Symbiont*Temp *Day, adjust="Tukey")
cld(emm)

emm<-emmeans(Singles_no0_D3D7_mod,  ~ Symbiont*Day|Temp, adjust="Tukey")
cld(emm)

```
 
Models same as above with day 14 31C removed
 
```{r} 
#Full model   
Singles_no0$daytemp<-paste0(Singles_no0$Day, Singles_no0$Temp )  #create tag
Singles_no0_NOD1431C<-subset(Singles_no0, daytemp !=1431)       #remove 1431


## lmer  symbionts*days*temp
Single_no0_all_mod_noD1431<-lmer(cells.larvae_log~Symbiont*Day*Temp+(1|PotCode), Singles_no0_NOD1431C)
anova(Single_no0_all_mod_noD1431) 
qqPlot(residuals(Single_no0_all_mod_noD1431))   # good

  emm<-emmeans(Single_no0_all_mod_noD1431,  ~ Symbiont|Temp *Day, adjust="Tukey")
cld(emm)
pairs(emm)

emm<-emmeans(Single_no0_all_mod_noD1431,  ~ Temp*Day|Symbiont , adjust="Tukey")
cld(emm)
pairs(emm)

emm<-emmeans(Single_no0_all_mod_noD1431,  ~ Symbiont*Day|Temp , adjust="Tukey")
cld(emm)
pairs(emm)
...
27C
```{r}     
Singles_no0_27<-subset(Singles_no0, Temp=="27")

Singles_no0_27_mean <- ddply(Singles_no0_27, c("Day", "Symbiont"), summarise, #summarize cell counts
                  N    = length(cells.larvae[!is.na(cells.larvae)]), #calculate the length of the data frame,  excluding NA’s *this part is important*
                 mean = mean(cells.larvae, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(cells.larvae, na.rm=TRUE), #calculate standard deviation 
                 se   = sd / sqrt(N), #calculate standard error
...
Singles_no0_27_mean #display table

#change Day back to Numeric from Factor, reapply the actual numeric dates
Singles_no0_27_mean$Day.num<-as.numeric(Singles_no0_27_mean$Day)
Singles_no0_27_mean$Day.num[Singles_no0_27_mean$Day.num == 2] =7
Singles_no0_27_mean$Day.num[Singles_no0_27_mean$Day.num == 3] =14
Singles_no0_27_mean$Day.num[Singles_no0_27_mean$Day.num == 1] =3
 
#plot with yaxis log10, real data (not transformed)
Singles_no0_27_mean_plot<-ggplot(data=Singles_no0_27_mean, aes(x=Day.num, y=mean, group = Symbiont, color=Symbiont)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Symbiont), size=4, show.legend = T)+  
  geom_line(aes(color=Symbiont),linetype="dotted")+
  ...
  ggtitle("27C Cell Counts")+
  theme(plot.title = element_text(size=20, face = "italic", hjust=0.5));Singles_no0_27_mean_plot
``` 
30C
```{r}      
Singles_no0_30<-subset(Singles_no0, Temp=="30")

Singles_no0_30_mean <- ddply(Singles_no0_30, c("Day", "Symbiont"), summarise, #summarize cell counts
                  N    = length(cells.larvae[!is.na(cells.larvae)]), #calculate the length of the data frame,  excluding NA’s *this part is important*
                 mean = mean(cells.larvae, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(cells.larvae, na.rm=TRUE), #calculate standard deviation 
                 se   = sd / sqrt(N), #calculate standard error
...
Singles_no0_30_mean #display table

#change Day back to Numeric from Factor, reapply the actual numeric dates
Singles_no0_30_mean$Day.num<-as.numeric(Singles_no0_30_mean$Day)
Singles_no0_30_mean$Day.num[Singles_no0_30_mean$Day.num == 2] =7
Singles_no0_30_mean$Day.num[Singles_no0_30_mean$Day.num == 3] =14
Singles_no0_30_mean$Day.num[Singles_no0_30_mean$Day.num == 1] =3
  
#plot with yaxis log10, real data (not transformed)
Singles_no0_30_mean_plot<-ggplot(data=Singles_no0_30_mean, aes(x=Day.num, y=mean, group = Symbiont, color=Symbiont)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Symbiont), size=4, show.legend = T)+  
  geom_line(aes(color=Symbiont),linetype="dotted")+
  ...
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("cells/larvae"))) +
  ggtitle("30C Cell Counts")+
  theme(plot.title = element_text(size=20, face = "italic", hjust=0.5));Singles_no0_30_mean_plot
``` 
31C
```{r}      
Singles_no0_31<-subset(Singles_no0, Temp=="31")

Singles_no0_31_mean <- ddply(Singles_no0_31, c("Day", "Symbiont"), summarise, #summarize cell counts
                  N    = length(cells.larvae[!is.na(cells.larvae)]), #calculate the length of the data frame,  excluding NA’s *this part is important*
                 mean = mean(cells.larvae, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(cells.larvae, na.rm=TRUE), #calculate standard deviation 
                 se   = sd / sqrt(N), #calculate standard error
...
Singles_no0_31_mean #display table

#change Day back to Numeric from Factor, reapply the actual numeric dates
Singles_no0_31_mean$Day.num<-as.numeric(Singles_no0_31_mean$Day)
Singles_no0_31_mean$Day.num[Singles_no0_31_mean$Day.num == 2] =7
Singles_no0_31_mean$Day.num[Singles_no0_31_mean$Day.num == 3] =14
Singles_no0_31_mean$Day.num[Singles_no0_31_mean$Day.num == 1] =3

#remove Day 14 due to low ns
Singles_no0_31_mean<-subset(Singles_no0_31_mean, Day=="3"|Day=="7")

#plot with yaxis log10, real data (not transformed)
Singles_no0_31_mean_plot<-ggplot(data=Singles_no0_31_mean, aes(x=Day.num, y=mean, group = Symbiont, color=Symbiont)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Symbiont), size=4, show.legend = T)+  
  geom_line(aes(color=Symbiont),linetype="dotted")+
  ...
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("cells/larvae"))) +
  ggtitle("31C Cell Counts")+
  theme(plot.title = element_text(size=20, face = "italic", hjust=0.5));Singles_no0_31_mean_plot
``` 

Plot all four SINGLE INFECTION together: Figure 1 20200822
```{r} 
#log scale axis only   
Singles_no0_27_mean_plot<-Singles_no0_27_mean_plot+ theme(legend.position = "none") #remove legend
Singles_no0_30_mean_plot<-Singles_no0_30_mean_plot+ theme(legend.position = "none") #remove legend
Singles_no0_31_mean_plot<-Singles_no0_31_mean_plot+ theme(legend.position = "none") #remove legend
grid.arrange(Singles_no0_27_mean_plot, Singles_no0_30_mean_plot, Singles_no0_31_mean_plot, nrow = 1) 
```

Plots By genus for your own benefit
C1
```{r}     
Singles_no0_C1<-subset(Singles_no0, Symbiont=="C1")
...
D1
```{r}     
Singles_no0_D1<-subset(Singles_no0, Symbiont=="D1")
...
F1
```{r}     
Singles_no0_F1<-subset(Singles_no0, Symbiont=="F1")
...
G3
```{r}     
Singles_no0_G3<-subset(Singles_no0, Symbiont=="G3")
...
ALL
```{r}     
Singles_no0_ALL<-subset(Singles_no0, Symbiont=="All")
...
# Monoculture vs poluculture-infection: Cell Counts PLOTS (Blue and pink plots) No 0s 20200824
C1         
...
SingleCounts<-SingleCounts[-c(5:8)] #remove metadata
...
CountsCompare2_mean
#######C1 31C day 7 only has 2 larva with non-0s. you should note it or cut it. 
...
#save
ggsave(filename="Figures/PinkblueCounts2.pdf", plot=InfectRaw_Fig1, dpi=300, width=12, height=17, units="in")
```
  
# Monoculture vs polyculture: Pink and Blue Models by symbiont, 0s removed for multi within specific zoox. YOU need this
Updated 20210105
```{r}           
#stats, log10: Normal 
...
hist(CountsCompare2_C1$cells.larvae_round) #
hist(CountsCompare2_C1$cells.larvae_log)


CountsCompare2_D1<-subset(CountsCompare2, Symbiont=="D1") 
hist(CountsCompare2_D1$cells.larvae_round) #
hist(CountsCompare2_D1$cells.larvae_log)


CountsCompare2_F1<-subset(CountsCompare2, Symbiont=="F1")
hist(CountsCompare2_F1$cells.larvae_round) #
hist(CountsCompare2_F1$cells.larvae_log)

CountsCompare2_G3<-subset(CountsCompare2, Symbiont=="G3")
hist(CountsCompare2_G3$cells.larvae_round) #
hist(CountsCompare2_G3$cells.larvae_log)
 

#Full model single v multi  <- not necessary
#remove day 14 31C 
#CountsCompare2_NoD1431C<-CountsCompare2
#CountsCompare2_NoD1431C$daytemp<-paste0(CountsCompare2$Day, CountsCompare2$Temp ) 
#  CountsCompare2_NoD1431C<-subset(CountsCompare2_NoD1431C, daytemp !=1431)       #remove 1431

#CountsCompare2_ALL_mod<-lmer(cells.larvae_log~Temp*Day*single.multi*Symbiont+(1|PotCode),Count#sCompare2_NoD1431C)
#anova(CountsCompare2_ALL_mod)
#qqPlot(residuals(CountsCompare2_ALL_mod))   # good
  
#emm<-emmeans(CountsCompare2_ALL_mod,  ~ Temp*Day*single.multi|Symbiont, adjust="Tukey")
#cld(emm)
#pairs(emm) 
...
#27C
CountsCompare2_C1_27<-subset(CountsCompare2_C1, Temp=="27")
CountsCompare2_C1_mod_27<-lmer(cells.larvae_log~Day*single.multi+(1|PotCode),CountsCompare2_C1_27)
anova(CountsCompare2_C1_mod_27)
qqPlot(residuals(CountsCompare2_C1_mod_27))   # good
   emm<-emmeans(CountsCompare2_C1_mod_27,  ~ single.multi|Day, adjust="Tukey") 
cld(emm)
pairs(emm)

#30C
CountsCompare2_C1_30<-subset(CountsCompare2_C1, Temp=="30")
CountsCompare2_C1_mod_30<-lmer(cells.larvae_log~Day*single.multi+(1|PotCode),CountsCompare2_C1_30)
anova(CountsCompare2_C1_mod_30)
qqPlot(residuals(CountsCompare2_C1_mod_30))   # good
   emm<-emmeans(CountsCompare2_C1_mod_30,  ~ single.multi|Day, adjust="Tukey") 
cld(emm)
pairs(emm)

#31C- JUST DAY 3
CountsCompare2_C1_31<-subset(CountsCompare2_C1, Temp=="31"& Day=="3")
CountsCompare2_C1_mod_31<-lmer(cells.larvae_log~single.multi+(1|PotCode),CountsCompare2_C1_31)
anova(CountsCompare2_C1_mod_31)
qqPlot(residuals(CountsCompare2_C1_mod_31))   # good
   emm<-emmeans(CountsCompare2_C1_mod_31,  ~ single.multi, adjust="Tukey") 
cld(emm)
pairs(emm)
...
D1, full mod, then by temp (you can just do by genus)
```{r}
...
#27C
CountsCompare2_D1_27<-subset(CountsCompare2_D1, Temp=="27")
CountsCompare2_D1_mod_27<-lmer(cells.larvae_log~Day*single.multi+(1|PotCode),CountsCompare2_D1_27)
anova(CountsCompare2_D1_mod_27)
qqPlot(residuals(CountsCompare2_D1_mod_27))   # good
   emm<-emmeans(CountsCompare2_D1_mod_27,  ~ single.multi|Temp|Day, adjust="Tukey") 
cld(emm)
pairs(emm)

#30C
CountsCompare2_D1_30<-subset(CountsCompare2_D1, Temp=="30")
CountsCompare2_D1_mod_30<-lmer(cells.larvae_log~Day*single.multi+(1|PotCode),CountsCompare2_D1_30)
anova(CountsCompare2_D1_mod_30)
qqPlot(residuals(CountsCompare2_D1_mod_30))   # good
   emm<-emmeans(CountsCompare2_D1_mod_30,  ~ single.multi|Temp|Day, adjust="Tukey") 
cld(emm)
pairs(emm)

#31C
CountsCompare2_D1_31<-subset(CountsCompare2_D1, Temp=="31")
CountsCompare2_D1_mod_31<-lmer(cells.larvae_log~Day*single.multi+(1|PotCode),CountsCompare2_D1_31)
anova(CountsCompare2_D1_mod_31)
qqPlot(residuals(CountsCompare2_D1_mod_31))   # good
   emm<-emmeans(CountsCompare2_D1_mod_31,  ~ single.multi|Temp|Day, adjust="Tukey") 
cld(emm)
pairs(emm)
...
qqPlot(residuals(CountsCompare2_F1_mod))   # ok
...
#27C
CountsCompare2_F1_27<-subset(CountsCompare2_F1, Temp=="27")
CountsCompare2_F1_mod_27<-lmer(cells.larvae_log~Day*single.multi+(1|PotCode),CountsCompare2_F1_27)
anova(CountsCompare2_F1_mod_27)
qqPlot(residuals(CountsCompare2_F1_mod_27))   # good
   emm<-emmeans(CountsCompare2_F1_mod_27,  ~ single.multi|Temp|Day, adjust="Tukey") 
cld(emm)
pairs(emm)

#30C
CountsCompare2_F1_30<-subset(CountsCompare2_F1, Temp=="30")
CountsCompare2_F1_mod_30<-lmer(cells.larvae_log~Day*single.multi+(1|PotCode),CountsCompare2_F1_30)
anova(CountsCompare2_F1_mod_30)
qqPlot(residuals(CountsCompare2_F1_mod_30))   # good
   emm<-emmeans(CountsCompare2_F1_mod_30,  ~ single.multi|Temp|Day, adjust="Tukey") 
cld(emm)
pairs(emm)

#31C
CountsCompare2_F1_31<-subset(CountsCompare2_F1, Temp=="31" )
CountsCompare2_F1_31<-subset(CountsCompare2_F1_31, Day=="3"|Day=="7" )

CountsCompare2_F1_mod_31<-lmer(cells.larvae_log~Day*single.multi+(1|PotCode),CountsCompare2_F1_31)
anova(CountsCompare2_F1_mod_31)
qqPlot(residuals(CountsCompare2_F1_mod_31))   # good
   emm<-emmeans(CountsCompare2_F1_mod_31,  ~ single.multi|Temp|Day, adjust="Tukey") 
cld(emm)
pairs(emm)
...
#27C
CountsCompare2_G3_27<-subset(CountsCompare2_G3, Temp=="27")
CountsCompare2_G3_mod_27<-lmer(cells.larvae_log~Day*single.multi+(1|PotCode),CountsCompare2_G3_27)
anova(CountsCompare2_G3_mod_27)
qqPlot(residuals(CountsCompare2_G3_mod_27))   # good
   emm<-emmeans(CountsCompare2_G3_mod_27,  ~ single.multi|Day, adjust="Tukey") 
cld(emm)
pairs(emm)

#30C
CountsCompare2_G3_30<-subset(CountsCompare2_G3, Temp=="30")
CountsCompare2_G3_mod_30<-lmer(cells.larvae_log~Day*single.multi+(1|PotCode),CountsCompare2_G3_30)
anova(CountsCompare2_G3_mod_30)
qqPlot(residuals(CountsCompare2_G3_mod_30))   # good
   emm<-emmeans(CountsCompare2_G3_mod_30,  ~ single.multi|Day, adjust="Tukey") 
cld(emm)
pairs(emm)

#31C
CountsCompare2_G3_31<-subset(CountsCompare2_G3, Temp=="31")
CountsCompare2_G3_31<-subset(CountsCompare2_G3, Day=="3"|Day=="7")

CountsCompare2_G3_mod_31<-lmer(cells.larvae_log~Day*single.multi+(1|PotCode),CountsCompare2_G3_31)
anova(CountsCompare2_G3_mod_31)
qqPlot(residuals(CountsCompare2_G3_mod_31))   # good
   emm<-emmeans(CountsCompare2_G3_mod_31,  ~ single.multi|Day, adjust="Tukey") 
cld(emm)
pairs(emm)
...
# B PLOT Polyculture infection, % larvae infected with 0,1, 2, 3, or 4 clades (not by clade, by number of clades). "rel1"= 1 clade,..  
```{r}           
...
#write.csv(ALL_RelA_Infect2,"ALL_RelA_Infect2_20200824.csv")


NumClades<-read.csv("NumberofGeneraperLarva_20200824.csv") #you couldn't do this in R so you did in excel and are reuploading....ugh. 

#you are trying this new again (old is saved below incase) 20200824 relative abundance of numclade bygrouping
...
#write.csv(NumCladesB_mean,"NumCladesB_mean20201014.csv")

#do only by Number of clades (no temp time) for how many larvae with 3 or 4 genera
...
# old file NumClades<-read.csv("SC18_numberofcladesinfect_20200625.csv") #you couldn't do this in R so you did in excel and are reuploading....ugh.NumberofGeneraperLarva_20200824.xlsx
...
Stats for Numclades~temp*time
```{r} 
# df RelA_CladeComp
NumClades_stats<-RelA_CladeComp # make a copy
...
# Distribution of numclade variable
ggdensity(NumClades_stats, x = "numclade", fill = "lightgray", title = "number of clades") +
  scale_x_continuous(limits = c(0, 4)) +
  stat_overlay_normal_density(color = "red", linetype = "dashed")

# Distribution of numclade variable2
ggdensity(NumClades_stats, x = "numclade_log2", fill = "lightgray", title = "number of clades") +
  scale_x_continuous(limits = c(0, 1)) +
  stat_overlay_normal_density(color = "red", linetype = "dashed")

#Full model   <- Not normal
## lmer  symbionts*days*temp
NumClades_stats_mod<-lmer(numclade_log2~Day*Temp+(1|PotCode), NumClades_stats)
anova(NumClades_stats_mod) 
qqPlot(residuals(NumClades_stats_mod))   # poor 

#try with numclade as factor
#NumClades_stats$numclade<-as.factor(as.character(NumClades_stats$numclade)) won't run lmer

# Chi Sq non-parametric <-NOPE
NumClades_stats_mod<-glmer(numclade~Day*Temp+(1|PotCode), family="poisson", NumClades_stats)
Anova(NumClades_stats_mod) 
qqPlot(residuals(NumClades_stats_mod))   # poor 

#Kruskal test - try by creating day-temp code and by running by day
  #by code <- do NOT use this
  NumClades_stats$DTCode<-paste0(NumClades_stats$Temp, NumClades_stats$Day)
  NumClades_stats$DTCode<-as.factor(NumClades_stats$DTCode)
  kruskal.test(numclade~DTCode, data=NumClades_stats)
...
Plots new bar plots Grey scale, FIND THE OTHER ONES inthe script. 
```{r}
### STACKED BAR PLOTS 
#27C 
...
Ok do you actually need this or can you use the rel abound for 3 and 4 to pull out the winner genera?
THIS MIGHt be the issue: 20201019 figure this out
** WHICH Clade winners: which genus was dominant in larvae on avg in each infection number group?
```{r}
NumWinner<-read.csv("SC18_numGeneraInfect.csv")
#remove 0s (code called C1 'winner' for 0s C1 by default)
NumWinner<-NumWinner[NumWinner$NumGenera != 0, ]

T27_D<-subset(NumWinner, Temp=="27")
T27_D3<-subset(T27_D, Day=="3")
T27_D7<-subset(T27_D, Day=="7")
T27_D14<-subset(T27_D, Day=="14")

# what percent of the total by these groupings is each genus?
T27_D3_rel<-T27_D3 %>% group_by(NumGenera) %>% mutate(relAbundByPath = numblarv / sum(numblarv))
T27_D3_rel<-subset(T27_D3_rel, select = -c(numblarv) )
names(T27_D3_rel)[names(T27_D3_rel)=="relAbundByPath"] <- "RelA"
T27_D3_relc<-dcast(T27_D3_rel, Temp+Day+NumGenera~NumberWinners)

T27_D7_rel<-T27_D7 %>% group_by(NumGenera) %>% mutate(relAbundByPath = numblarv / sum(numblarv))
T27_D7_rel<-subset(T27_D7_rel, select = -c(numblarv) )
names(T27_D7_rel)[names(T27_D7_rel)=="relAbundByPath"] <- "RelA"
T27_D7_relc<-dcast(T27_D7_rel, Temp+Day+NumGenera~NumberWinners)

T27_D14_rel<-T27_D14 %>% group_by(NumGenera) %>% mutate(relAbundByPath = numblarv / sum(numblarv))
T27_D14_rel<-subset(T27_D14_rel, select = -c(numblarv) )
names(T27_D14_rel)[names(T27_D14_rel)=="relAbundByPath"] <- "RelA"
T27_D14_relc<-dcast(T27_D14_rel, Temp+Day+NumGenera~NumberWinners)

# barplot 27 D3  <- this gives us winner of all genera infections but we only want 3 and 4 bc so few 0-2
T27_D3_rel_plot<-ggplot(T27_D3_rel, aes(y=RelA, x=NumGenera, fill=NumberWinners)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of General")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        axis.title=element_text(size=14,face="bold"), #Set axis format
        panel.border = element_blank(), #Set the border
  ...
  ggtitle("27C day 3");T27_D3_rel_plot   

# barplot 27 D7
T27_D7_rel_plot<-ggplot(T27_D7_rel, aes(y=RelA, x=NumGenera, fill=NumberWinners)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of General")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("RelAbund"))) +
  ggtitle("27C day 7");T27_D7_rel_plot  

# barplot 27 D14
T27_D14_rel_plot<-ggplot(T27_D14_rel, aes(y=RelA, x=NumGenera, fill=NumberWinners)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of General")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("RelAbund"))) +
  ggtitle("27C day 14");T27_D14_rel_plot  


#30C
T30_D<-subset(NumWinner, Temp=="30")
T30_D3<-subset(T30_D, Day=="3")
T30_D7<-subset(T30_D, Day=="7")
T30_D14<-subset(T30_D, Day=="14")

T30_D3_rel<-T30_D3 %>% group_by(NumGenera) %>% mutate(relAbundByPath = numblarv / sum(numblarv))
T30_D3_rel<-subset(T30_D3_rel, select = -c(numblarv) )
names(T30_D3_rel)[names(T30_D3_rel)=="relAbundByPath"] <- "RelA"
T30_D3_relc<-dcast(T30_D3_rel, Temp+Day+NumGenera~NumberWinners)

T30_D7_rel<-T30_D7 %>% group_by(NumGenera) %>% mutate(relAbundByPath = numblarv / sum(numblarv))
T30_D7_rel<-subset(T30_D7_rel, select = -c(numblarv) )
names(T30_D7_rel)[names(T30_D7_rel)=="relAbundByPath"] <- "RelA"
T30_D7_relc<-dcast(T30_D7_rel, Temp+Day+NumGenera~NumberWinners)

T30_D14_rel<-T30_D14 %>% group_by(NumGenera) %>% mutate(relAbundByPath = numblarv / sum(numblarv))
T30_D14_rel<-subset(T30_D14_rel, select = -c(numblarv) )
names(T30_D14_rel)[names(T30_D14_rel)=="relAbundByPath"] <- "RelA"
T30_D14_relc<-dcast(T30_D14_rel, Temp+Day+NumGenera~NumberWinners)

# barplot 30 D3 
T30_D3_rel_plot<-ggplot(T30_D3_rel, aes(y=RelA, x=NumGenera, fill=NumberWinners)) + 
           geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of General")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("RelAbund"))) +
  ggtitle("30C day 3");T30_D3_rel_plot   

# barplot 30 D7
T30_D7_rel_plot<-ggplot(T30_D7_rel, aes(y=RelA, x=NumGenera, fill=NumberWinners)) + 
           geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#EEA236FF","#46B8DAFF"))+
   xlab("Number of General")+
         scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
 ...
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("RelAbund"))) +
  ggtitle("30C day 7");T30_D7_rel_plot  

# barplot 30 D14
T30_D14_rel_plot<-ggplot(T30_D14_rel, aes(y=RelA, x=NumGenera, fill=NumberWinners)) + 
 ...
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("RelAbund"))) +
  ggtitle("30C day 14");T30_D14_rel_plot 



#31C
T31_D<-subset(NumWinner, Temp=="31")
T31_D3<-subset(T31_D, Day=="3")
T31_D7<-subset(T31_D, Day=="7")

T31_D3_rel<-T31_D3 %>% group_by(NumGenera) %>% mutate(relAbundByPath = numblarv / sum(numblarv))
T31_D3_rel<-subset(T31_D3_rel, select = -c(numblarv) )
names(T31_D3_rel)[names(T31_D3_rel)=="relAbundByPath"] <- "RelA"
T31_D3_relc<-dcast(T30_D14_rel, Temp+Day+NumGenera~NumberWinners)

T31_D7_rel<-T31_D7 %>% group_by(NumGenera) %>% mutate(relAbundByPath = numblarv / sum(numblarv))
T31_D7_rel<-subset(T31_D7_rel, select = -c(numblarv) )
names(T31_D7_rel)[names(T31_D7_rel)=="relAbundByPath"] <- "RelA"
T31_D7_relc<-dcast(T30_D14_rel, Temp+Day+NumGenera~NumberWinners)



# barplot 31 D3 
T31_D3_rel_plot<-ggplot(T31_D3_rel, aes(y=RelA, x=NumGenera, fill=NumberWinners)) + 
geom_bar(stat="identity", width = .6)+  
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of General")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("RelAbund"))) +
  ggtitle("31C day 3");T31_D3_rel_plot   

# barplot 31 D7
T31_D7_rel_plot<-ggplot(T31_D7_rel, aes(y=RelA, x=NumGenera, fill=NumberWinners)) + 
geom_bar(stat="identity", width = .6)+ 
  scale_fill_manual(values=c("#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(1,5))+
   xlab("Number of General")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("RelAbund"))) +
  ggtitle("31C day 7");T31_D7_rel_plot  

T27_D3_rel_plot<-T27_D3_rel_plot+ theme(legend.position = "none") #remove legend
T27_D7_rel_plot<-T27_D7_rel_plot+ theme(legend.position = "none") #remove legend
T27_D14_rel_plot<-T27_D14_rel_plot+ theme(legend.position = "none") #remove legend

T30_D3_rel_plot<-T30_D3_rel_plot+ theme(legend.position = "none") #remove legend
T30_D7_rel_plot<-T30_D7_rel_plot+ theme(legend.position = "none") #remove legend
T30_D14_rel_plot<-T30_D14_rel_plot+ theme(legend.position = "none") #remove legend

T31_D3_rel_plot<-T31_D3_rel_plot+ theme(legend.position = "none") #remove legend
T31_D7_rel_plot<-T31_D7_rel_plot+ theme(legend.position = "none") #remove legend

grid.arrange(T27_D3_rel_plot, T27_D7_rel_plot, T27_D14_rel_plot,T30_D3_rel_plot,T30_D7_rel_plot,T30_D14_rel_plot, T31_D3_rel_plot,T31_D7_rel_plot,nrow = 3)

#combine all separate 
#not all genera represented so add in genera with 0s 
T27_D7_relc$F1<-0
T27_D14_relc$F1<-0
T30_D7_relc$F1<-0
T30_D7_relc$C1<-0
...
#% of winning genera
GeneraWinners<-rbind(T27_D3_relc, T27_D7_relc,T27_D14_relc,T30_D3_relc, T30_D7_relc,T30_D14_relc,T31_D3_relc, T31_D7_relc)



```

Now lets do relA for each grouping of number of genera/larva  
df: ALL_RelA_b  (relative abundance, the cells.larvae=Rel abund)
```{r} 
aData<-ALL_RelA2
  aData$C1[aData$C1>0]<-1  #make new cols for infect or not 1 for infect, 0 not.
  aData$C1[aData$C1==0]<-0
    aData$D1[aData$D1>0]<-1  #make new cols for infect or not 1 for infect, 0 not.
  aData$D1[aData$D1==0]<-0
    aData$F1[aData$F1>0]<-1  #make new cols for infect or not 1 for infect, 0 not.
  aData$F1[aData$F1==0]<-0
    aData$G3[aData$G3>0]<-1  #make new cols for infect or not 1 for infect, 0 not.
  aData$G3[aData$G3==0]<-0
...
aData2<-aData[c("ALL.number","numbClades")] #new row with the sum of num of genera infected 
#combine this with the Rel Abund dataframe (ALL_RelA_b)

aData3<-merge(aData2,ALL_RelA_b, by="ALL.number")




## THIS IS RELATIVE ABUNDANCES FOR 3 AND 4 GENERA LARVA (NOT DOMINANT CLADE)
#27C
NumWinner_27RelA<-subset(aData3, Temp==27) #subset temp 27

#D3
NumWinner_27RelA_D3<-subset(NumWinner_27RelA, Day==3) #subset day 3
NumWinner_27RelA_D3a<-NumWinner_27RelA_D3[c("numbClades","Symbiont","cells.larvae")] #keep only cols you need, btw cells. larvae is actually relative abundance
NumWinner_27RelA_D3a_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_27RelA_D3a, mean) #aggregate by symbiont + numbclades

  #27 Day 3 by 3  <- use to make new spreadsheet with relA for 3-gen and 4-gen
  NumWinner_27RelA_D3b<-NumWinner_27RelA_D3[c("numbClades","Symbiont","cells.larvae")]
  NumWinner_27RelA_D3b_3gen<-subset(NumWinner_27RelA_D3b, numbClades=="3")
  NumWinner_27RelA_D3b_3gen_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_27RelA_D3b_3gen, mean)
  NumWinner_27RelA_D3b_3gen$Day<-3
    NumWinner_27RelA_D3b_3gen$Temp<-27
    NumWinner_27RelA_D3b_3gen$genera<-3

  
   #27 Day 3 by 4
  NumWinner_27RelA_D3b_4gen<-subset(NumWinner_27RelA_D3b, numbClades=="4")
  NumWinner_27RelA_D3b_4gen_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_27RelA_D3b_4gen, mean)
NumWinner_27RelA_D3b_4gen$Day<-3
NumWinner_27RelA_D3b_4gen$Temp<-27
    NumWinner_27RelA_D3b_4gen$genera<-4
    
  testing<-rbind(NumWinner_27RelA_D3b_3gen,NumWinner_27RelA_D3b_4gen)



#D7
NumWinner_27RelA_D7<-subset(NumWinner_27RelA, Day==7) #subset day 7
NumWinner_27RelA_D7a<-NumWinner_27RelA_D7[c("numbClades","Symbiont","cells.larvae")] #keep only cols you need
NumWinner_27RelA_D7a_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_27RelA_D7a, mean) #aggregate by symbiont + numbclades

 #27 Day 7 by 3 
  NumWinner_27RelA_D7b<-NumWinner_27RelA_D7[c("numbClades","Symbiont","cells.larvae")]
  NumWinner_27RelA_D7b_3gen<-subset(NumWinner_27RelA_D7b, numbClades=="3")
  NumWinner_27RelA_D7b_3gen_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_27RelA_D7b_3gen, mean)
  
   #27 Day 7 by 4
  NumWinner_27RelA_D7b_4gen<-subset(NumWinner_27RelA_D7b, numbClades=="4")
  NumWinner_27RelA_D7b_4gen_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_27RelA_D7b_4gen, mean)

   #27 Day 7 by 2 genera 
  NumWinner_27RelA_D7b_2gen<-subset(NumWinner_27RelA_D7b, numbClades=="2")
  NumWinner_27RelA_D7b_2gen_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_27RelA_D7b_2gen, mean)


#D14
NumWinner_27RelA_D14<-subset(NumWinner_27RelA, Day==14) #subset day 14
NumWinner_27RelA_D14a<-NumWinner_27RelA_D14[c("numbClades","Symbiont","cells.larvae")] #keep only cols you need
NumWinner_27RelA_D14a_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_27RelA_D14a, mean) #aggregate by symbiont + numbclades

 #27 Day 14 by 3 
  NumWinner_27RelA_D14b<-NumWinner_27RelA_D14[c("numbClades","Symbiont","cells.larvae")]
  NumWinner_27RelA_D14b_3gen<-subset(NumWinner_27RelA_D14b, numbClades=="3")
  NumWinner_27RelA_D14b_3gen_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_27RelA_D14b_3gen, mean)
  
   #27 Day 14 by 4
  NumWinner_27RelA_D14b_4gen<-subset(NumWinner_27RelA_D14b, numbClades=="4")
  NumWinner_27RelA_D14b_4gen_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_27RelA_D14b_4gen, mean)





#Plots relative abundance by # of genera
# barplot 27 D3 
NumWinner_27RelA_D3a_agg_plot<-ggplot(NumWinner_27RelA_D3a_agg, aes(y=cells.larvae,x=numbClades, fill=Symbiont)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
 ...
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("RelAbund"))) +
  ggtitle("27C day 3");NumWinner_27RelA_D3a_agg_plot   

# barplot 27 D7 
NumWinner_27RelA_D7_agg_plot<-ggplot(NumWinner_27RelA_D7a_agg, aes(y=cells.larvae,x=numbClades, fill=Symbiont)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of General")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("RelAbund"))) +
  ggtitle("27C day 7");NumWinner_27RelA_D7_agg_plot   

# barplot 27 D14
NumWinner_27RelA_D14_agg_plot<-ggplot(NumWinner_27RelA_D14a_agg, aes(y=cells.larvae,x=numbClades, fill=Symbiont)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of General")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("RelAbund"))) +
  ggtitle("27C day 14");NumWinner_27RelA_D14_agg_plot   


#30C

#30C
NumWinner_30RelA<-subset(aData3, Temp==30) #subset temp 30
#D3
NumWinner_30RelA_D3<-subset(NumWinner_30RelA, Day==3) #subset day 3
NumWinner_30RelA_D3a<-NumWinner_30RelA_D3[c("numbClades","Symbiont","cells.larvae")] #keep only cols you need
NumWinner_30RelA_D3a_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_30RelA_D3a, mean) #aggregate by symbiont + numbclades

#30 Day 3 by 3  <- use to make new spreadsheet with relA for 3-gen and 4-gen
  NumWinner_30RelA_D3b<-NumWinner_30RelA_D3[c("numbClades","Symbiont","cells.larvae")]
  NumWinner_30RelA_D3b_3gen<-subset(NumWinner_30RelA_D3b, numbClades=="3")
  NumWinner_30RelA_D3b_3gen_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_30RelA_D3b_3gen, mean)
  
   #30 Day 3 by 4
  NumWinner_30RelA_D3b_4gen<-subset(NumWinner_30RelA_D3b, numbClades=="4")
  NumWinner_30RelA_D3b_4gen_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_30RelA_D3b_4gen, mean)


#D7
NumWinner_30RelA_D7<-subset(NumWinner_30RelA, Day==7) #subset day 7
NumWinner_30RelA_D7a<-NumWinner_30RelA_D7[c("numbClades","Symbiont","cells.larvae")] #keep only cols you need
NumWinner_30RelA_D7a_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_30RelA_D7a, mean) #aggregate by symbiont + numbclades

#30 Day 7 by 3 
  NumWinner_30RelA_D7b<-NumWinner_30RelA_D7[c("numbClades","Symbiont","cells.larvae")]
  NumWinner_30RelA_D7b_3gen<-subset(NumWinner_30RelA_D7b, numbClades=="3")
  NumWinner_30RelA_D7b_3gen_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_30RelA_D7b_3gen, mean)
  
   #30 Day 7 by 4
  NumWinner_30RelA_D7b_4gen<-subset(NumWinner_30RelA_D7b, numbClades=="4")
  NumWinner_30RelA_D7b_4gen_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_30RelA_D7b_4gen, mean)



#D14
NumWinner_30RelA_D14<-subset(NumWinner_30RelA, Day==14) #subset day 14
NumWinner_30RelA_D14a<-NumWinner_30RelA_D14[c("numbClades","Symbiont","cells.larvae")] #keep only cols you need
NumWinner_30RelA_D14a_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_30RelA_D14a, mean) #aggregate by symbiont + numbclades


#30 Day 14 by 3 
  NumWinner_30RelA_D14b<-NumWinner_30RelA_D14[c("numbClades","Symbiont","cells.larvae")]
  NumWinner_30RelA_D14b_3gen<-subset(NumWinner_30RelA_D14b, numbClades=="3")
  NumWinner_30RelA_D14b_3gen_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_30RelA_D14b_3gen, mean)
  
   #30 Day 14 by 4
  NumWinner_30RelA_D14b_4gen<-subset(NumWinner_30RelA_D14b, numbClades=="4")
  NumWinner_30RelA_D14b_4gen_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_30RelA_D14b_4gen, mean)



#Plots
# barplot 30 D3 
NumWinner_30RelA_D3a_agg_plot<-ggplot(NumWinner_30RelA_D3a_agg, aes(y=cells.larvae,x=numbClades, fill=Symbiont)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of General")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("RelAbund"))) +
  ggtitle("30C day 3");NumWinner_30RelA_D3a_agg_plot   

# barplot 30 D7 
NumWinner_30RelA_D7_agg_plot<-ggplot(NumWinner_30RelA_D7a_agg, aes(y=cells.larvae,x=numbClades, fill=Symbiont)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of General")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("RelAbund"))) +
  ggtitle("30C day 7");NumWinner_30RelA_D7_agg_plot   

# barplot 30 D14
NumWinner_30RelA_D14_agg_plot<-ggplot(NumWinner_30RelA_D14a_agg, aes(y=cells.larvae,x=numbClades, fill=Symbiont)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of General")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("RelAbund"))) +
  ggtitle("30C day 14");NumWinner_30RelA_D14_agg_plot   
...
#31C
NumWinner_31RelA<-subset(aData3, Temp==31) #subset temp 31
#D3
NumWinner_31RelA_D3<-subset(NumWinner_31RelA, Day==3) #subset day 3
NumWinner_31RelA_D3a<-NumWinner_31RelA_D3[c("numbClades","Symbiont","cells.larvae")] #keep only cols you need
NumWinner_31RelA_D3a_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_31RelA_D3a, mean) #aggregate by symbiont + numbclades

#31 Day 3 by 3  <- use to make new spreadsheet with relA for 3-gen and 4-gen
  NumWinner_31RelA_D3b<-NumWinner_31RelA_D3[c("numbClades","Symbiont","cells.larvae")]
  NumWinner_31RelA_D3b_3gen<-subset(NumWinner_31RelA_D3b, numbClades=="3")
  NumWinner_31RelA_D3b_3gen_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_31RelA_D3b_3gen, mean)
  
   #31 Day 3 by 4
  NumWinner_31RelA_D3b_4gen<-subset(NumWinner_31RelA_D3b, numbClades=="4")
  NumWinner_31RelA_D3b_4gen_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_31RelA_D3b_4gen, mean)



#D7
NumWinner_31RelA_D7<-subset(NumWinner_31RelA, Day==7) #subset day 7
NumWinner_31RelA_D7a<-NumWinner_31RelA_D7[c("numbClades","Symbiont","cells.larvae")] #keep only cols you need
NumWinner_31RelA_D7a_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_31RelA_D7a, mean) #aggregate by symbiont + numbclades

  #31 Day 3 by 3  <- use to make new spreadsheet with relA for 3-gen and 4-gen
  NumWinner_31RelA_D7b<-NumWinner_31RelA_D7[c("numbClades","Symbiont","cells.larvae")]
  NumWinner_31RelA_D7b_3gen<-subset(NumWinner_31RelA_D7b, numbClades=="3")
  NumWinner_31RelA_D7b_3gen_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_31RelA_D7b_3gen, mean)
  
   #31 Day 3 by 4
  NumWinner_31RelA_D7b_4gen<-subset(NumWinner_31RelA_D7b, numbClades=="4")
  NumWinner_31RelA_D7b_4gen_agg<-aggregate(.~Symbiont+numbClades, data=NumWinner_31RelA_D7b_4gen, mean)

#Plots
# barplot 31 D3 
NumWinner_31RelA_D3a_agg_plot<-ggplot(NumWinner_31RelA_D3a_agg, aes(y=cells.larvae,x=numbClades, fill=Symbiont)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of General")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("RelAbund"))) +
  ggtitle("31C day 3");NumWinner_31RelA_D3a_agg_plot   

# barplot 31 D7 
NumWinner_31RelA_D7_agg_plot<-ggplot(NumWinner_31RelA_D7a_agg, aes(y=cells.larvae,x=numbClades, fill=Symbiont)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of General")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("RelAbund"))) +
  ggtitle("31C day 7");NumWinner_31RelA_D7_agg_plot   



#plot together

NumWinner_27RelA_D3a_agg_plot<-NumWinner_27RelA_D3a_agg_plot+ theme(legend.position = "none") #remove legend
NumWinner_27RelA_D7_agg_plot<-NumWinner_27RelA_D7_agg_plot+ theme(legend.position = "none") #remove legend
NumWinner_27RelA_D14_agg_plot<-NumWinner_27RelA_D14_agg_plot+ theme(legend.position = "none") #remove legend

NumWinner_30RelA_D3a_agg_plot<-NumWinner_30RelA_D3a_agg_plot+ theme(legend.position = "none") #remove legend
NumWinner_30RelA_D7_agg_plot<-NumWinner_30RelA_D7_agg_plot+ theme(legend.position = "none") #remove legend
NumWinner_30RelA_D14_agg_plot<-NumWinner_30RelA_D14_agg_plot+ theme(legend.position = "none") #remove legend

NumWinner_31RelA_D3a_agg_plot<-NumWinner_31RelA_D3a_agg_plot+ theme(legend.position = "none") #remove legend
NumWinner_31RelA_D7_agg_plot<-NumWinner_31RelA_D7_agg_plot+ theme(legend.position = "none") #remove legend

grid.arrange(NumWinner_27RelA_D3a_agg_plot, NumWinner_27RelA_D7_agg_plot, NumWinner_27RelA_D14_agg_plot,NumWinner_30RelA_D3a_agg_plot,NumWinner_30RelA_D7_agg_plot,NumWinner_30RelA_D14_agg_plot, NumWinner_31RelA_D3a_agg_plot,NumWinner_31RelA_D7_agg_plot,nrow = 3)
...
#THIS SECTION IS RELATIVE ABUNDANCE OF LARVAE INFECTED WITH 3 OR 4 GENERA BY dom GENERA :
YOU are here!  Separeated out 3 and 4 genra groups. made mean table. figure out stats and new plots. note the single larvae with 4-genera in d7 31, do you want to include only groups with 3+. working: do all genera
Now figure out relA by genus-infection group dom genus:
```{r} 
# gives the relative abundance of larvae within 1, 2, 3 or 4 genera infected larvae. manually plotted by dom genus. 
#3gen
aData4<-aData3  #make copy



aData4_mean <- ddply(aData4, c("Day", "Temp","numbClades","Symbiont"), summarise, #summarize cell counts
                 N    = length(cells.larvae[!is.na(cells.larvae)]), #calculate the length of the data frame,  excluding NA’s *this part is important*
                 mean = mean(cells.larvae, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(cells.larvae, na.rm=TRUE), #calculate standard deviation 
                 se   = sd / sqrt(N), #calculate standard error
...
aData4_mean #display table 


#27
aData4_27<-subset(aData4, Temp==27)
#Day 3
  aData4_27_D3<-subset(aData4_27, Day==3)
    aData4_27_D3_g3<-subset(aData4_27_D3, numbClades==3)
    aData4_27_D3_g4<-subset(aData4_27_D3, numbClades==4)
    aData4_27_D3_g2<-subset(aData4_27_D3, numbClades==2)
    aData4_27_D3_g1<-subset(aData4_27_D3, numbClades==1)
#Day 7
  aData4_27_D7<-subset(aData4_27, Day==7)
    aData4_27_D7_g3<-subset(aData4_27_D7, numbClades==3)
    aData4_27_D7_g4<-subset(aData4_27_D7, numbClades==4)
    aData4_27_D7_g2<-subset(aData4_27_D7, numbClades==2)
    aData4_27_D7_g1<-subset(aData4_27_D7, numbClades==1)
 #Day 14   
  aData4_27_D14<-subset(aData4_27, Day==14)
    aData4_27_D14_g3<-subset(aData4_27_D14, numbClades==3)
    aData4_27_D14_g4<-subset(aData4_27_D14, numbClades==4)
    aData4_27_D14_g2<-subset(aData4_27_D14, numbClades==2)
    aData4_27_D14_g1<-subset(aData4_27_D14, numbClades==1)

#30
aData4_30<-subset(aData4, Temp==30)
#Day 3
aData4_30_D3<-subset(aData4_30, Day==3)
    aData4_30_D3_g3<-subset(aData4_30_D3, numbClades==3)
    aData4_30_D3_g4<-subset(aData4_30_D3, numbClades==4)
    aData4_30_D3_g2<-subset(aData4_30_D3, numbClades==2)
    aData4_30_D3_g1<-subset(aData4_30_D3, numbClades==1)
#Day 7
aData4_30_D7<-subset(aData4_30, Day==7)
    aData4_30_D7_g3<-subset(aData4_30_D7, numbClades==3)
    aData4_30_D7_g4<-subset(aData4_30_D7, numbClades==4)
    aData4_30_D7_g2<-subset(aData4_30_D7, numbClades==2)
    aData4_30_D7_g1<-subset(aData4_30_D7, numbClades==1)
#Day 14
aData4_30_D14<-subset(aData4_30, Day==14)
    aData4_30_D14_g3<-subset(aData4_30_D14, numbClades==3)
    aData4_30_D14_g4<-subset(aData4_30_D14, numbClades==4)
    aData4_30_D14_g2<-subset(aData4_30_D14, numbClades==2)
    aData4_30_D14_g1<-subset(aData4_30_D14, numbClades==1)
    
#31
aData4_31<-subset(aData4, Temp==31)
#Dzy 3
aData4_31_D3<-subset(aData4_31, Day==3)
    aData4_31_D3_g3<-subset(aData4_31_D3, numbClades==3)
    aData4_31_D3_g4<-subset(aData4_31_D3, numbClades==4)
    aData4_31_D3_g2<-subset(aData4_31_D3, numbClades==2)
    aData4_31_D3_g1<-subset(aData4_31_D3, numbClades==1)
#Day 7    
aData4_31_D7<-subset(aData4_31, Day==7)
    aData4_31_D7_g3<-subset(aData4_31_D7, numbClades==3)
    aData4_31_D7_g4<-subset(aData4_31_D7, numbClades==4)
    aData4_31_D7_g2<-subset(aData4_31_D7, numbClades==2)
    aData4_31_D7_g1<-subset(aData4_31_D7, numbClades==1)

# stacked bar - INDIVIDUAL LARVAE
    #teasting to plot them all in one go by temp 27

#27C
  aData4_27$NumGen_ID<-paste(aData4_27$numbClades,aData4_27$ALL.number)#new col combing num clade and ID
...
#30C
  aData4_30$NumGen_ID<-paste(aData4_30$numbClades,aData4_30$ALL.number)#new col combing num clade and ID
...
#31C
  aData4_31$NumGen_ID<-paste(aData4_31$numbClades,aData4_31$ALL.number)#new col combing num clade and ID
...
aData4_27_D14_g3_plot<-ggplot(aData4_27_D14_g3, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) +
 ...
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("27C_D14_3gen");aData4_27_D3_g3_plot
...
#D3 2gen
aData4_1or2_27_D3_g2_plot<-ggplot(aData4_1or2_27_D3_g4, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
           geom_bar(stat="identity", width=.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("27C_D3_2gen");aData4_1or2_27_D3_g2_plot

#day7  2gen
aData4_1or2_27_D7_g2_plot<-ggplot(aData4_1or2_27_D7_g2, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
           geom_bar(stat="identity", width=0.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("27C_D7_2gen");aData4_1or2_27_D7_g2_plot
...
#D3 1gen
aData4_1or2_27_D3_g1_plot<-ggplot(aData4_1or2_27_D3_g1, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
           geom_bar(stat="identity", width=.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("27C_D3_1gen");aData4_1or2_27_D3_g1_plot
...
backup of above, delete when sorted out
```{r} 
# gives the relative abundance of larvae within 3 or 4 genera infected larvae. manually plotted by dom genus. 
#3gen
aData4<-aData3  #make copy

#3 or 4 (main)
aData4_3or4<-subset(aData4, numbClades=="3"|numbClades=="4")

aData4_3or4_mean <- ddply(aData4_3or4, c("Day", "Temp","numbClades","Symbiont"), summarise, #summarize cell counts
                 N    = length(cells.larvae[!is.na(cells.larvae)]), #calculate the length of the data frame,  excluding NA’s *this part is important*
                 mean = mean(cells.larvae, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(cells.larvae, na.rm=TRUE), #calculate standard deviation 
                 se   = sd / sqrt(N), #calculate standard error
...
aData4_3or4_mean #display table 



#for 1 or 2 clades (checking)
  aData4_1or2<-subset(aData4, numbClades=="1"|numbClades=="2")

  aData4_1or2_mean <- ddply(aData4_1or2, c("Day", "Temp","numbClades","Symbiont"), summarise, #summarize cell counts
                 N    = length(cells.larvae[!is.na(cells.larvae)]), #calculate the length of the data frame,  excluding   NA’s *this part is important*
                 mean = mean(cells.larvae, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(cells.larvae, na.rm=TRUE), #calculate standard deviation 
                 se   = sd / sqrt(N), #calculate standard error
  ...
  aData4_1or2_mean #display table 



# cast
aData4_3or4_vars<-c("ALL.number", "numbClades", "Day","Temp","PotCode","Symbiont","cells.larvae")
aData4_3or4_cast<-aData4_3or4[aData4_3or4_vars]
aData4_3or4_cast2<-dcast(aData4_3or4_cast, ALL.number+numbClades+PotCode+Temp+Day~Symbiont) 

aData4_3or4_cast2_27C<-subset(aData4_3or4_cast2, Temp=="27")
aData4_3or4_cast2_27C_D3<-subset(aData4_3or4_cast2_27C, Day=="3")
aData4_3or4_cast2_27C_D7<-subset(aData4_3or4_cast2_27C, Day=="7")
aData4_3or4_cast2_27C_D14<-subset(aData4_3or4_cast2_27C, Day=="14")

aData4_3or4_cast2_30C<-subset(aData4_3or4_cast2, Temp=="30")
aData4_3or4_cast2_30C_D3<-subset(aData4_3or4_cast2_30C, Day=="3")
aData4_3or4_cast2_30C_D7<-subset(aData4_3or4_cast2_30C, Day=="7")
aData4_3or4_cast2_30C_D14<-subset(aData4_3or4_cast2_30C, Day=="14")

aData4_3or4_cast2_31C<-subset(aData4_3or4_cast2, Temp=="31")
aData4_3or4_cast2_31C_D3<-subset(aData4_3or4_cast2_31C, Day=="3")
aData4_3or4_cast2_31C_D7<-subset(aData4_3or4_cast2_31C, Day=="7")

#Same as above but no cast
#27
#3 or 4
aData4_3or4_27<-subset(aData4_3or4, Temp==27)
  aData4_3or4_27_D3<-subset(aData4_3or4_27, Day==3)
    aData4_3or4_27_D3_g3<-subset(aData4_3or4_27_D3, numbClades==3)
    aData4_3or4_27_D3_g4<-subset(aData4_3or4_27_D3, numbClades==4)

  aData4_3or4_27_D7<-subset(aData4_3or4_27, Day==7)
    aData4_3or4_27_D7_g3<-subset(aData4_3or4_27_D7, numbClades==3)
    aData4_3or4_27_D7_g4<-subset(aData4_3or4_27_D7, numbClades==4)
    
  aData4_3or4_27_D14<-subset(aData4_3or4_27, Day==14)
    aData4_3or4_27_D14_g3<-subset(aData4_3or4_27_D14, numbClades==3)
    aData4_3or4_27_D14_g4<-subset(aData4_3or4_27_D14, numbClades==4)
  #1 or 2  
    aData4_1or2_27<-subset(aData4_1or2, Temp==27)
  aData4_1or2_27_D3<-subset(aData4_1or2_27, Day==3)
    aData4_1or2_27_D3_g1<-subset(aData4_1or2_27_D3, numbClades==1)
    aData4_1or2_27_D3_g2<-subset(aData4_1or2_27_D3, numbClades==2)

  aData4_1or2_27_D7<-subset(aData4_1or2_27, Day==7)
    aData4_1or2_27_D7_g1<-subset(aData4_1or2_27_D7, numbClades==1)
    aData4_1or2_27_D7_g2<-subset(aData4_1or2_27_D7, numbClades==2)
    
  aData4_1or2_27_D14<-subset(aData4_1or2_27, Day==14)
    aData4_1or2_27_D14_g1<-subset(aData4_1or2_27_D14, numbClades==1)
    aData4_1or2_27_D14_g2<-subset(aData4_1or2_27_D14, numbClades==2)
    
#30
aData4_3or4_30<-subset(aData4_3or4, Temp==30)
aData4_3or4_30_D3<-subset(aData4_3or4_30, Day==3)
    aData4_3or4_30_D3_g3<-subset(aData4_3or4_30_D3, numbClades==3)
    aData4_3or4_30_D3_g4<-subset(aData4_3or4_30_D3, numbClades==4)
aData4_3or4_30_D7<-subset(aData4_3or4_30, Day==7)
    aData4_3or4_30_D7_g3<-subset(aData4_3or4_30_D7, numbClades==3)
    aData4_3or4_30_D7_g4<-subset(aData4_3or4_30_D7, numbClades==4)
aData4_3or4_30_D14<-subset(aData4_3or4_30, Day==14)
    aData4_3or4_30_D14_g3<-subset(aData4_3or4_30_D14, numbClades==3)
    aData4_3or4_30_D14_g4<-subset(aData4_3or4_30_D14, numbClades==4)
    
#1 or2
    aData4_1or2_30<-subset(aData4_1or2, Temp==30)
aData4_1or2_30_D3<-subset(aData4_1or2_30, Day==3)
    aData4_1or2_30_D3_g1<-subset(aData4_1or2_30_D3, numbClades==1)
    aData4_1or2_30_D3_g2<-subset(aData4_1or2_30_D3, numbClades==2)
aData4_1or2_30_D7<-subset(aData4_1or2_30, Day==7)
    aData4_1or2_30_D7_g1<-subset(aData4_1or2_30_D7, numbClades==1)
    aData4_1or2_30_D7_g2<-subset(aData4_1or2_30_D7, numbClades==2)
aData4_1or2_30_D14<-subset(aData4_1or2_30, Day==14)
    aData4_1or2_30_D14_g1<-subset(aData4_1or2_30_D14, numbClades==1)
    aData4_1or2_30_D14_g2<-subset(aData4_1or2_30_D14, numbClades==2)
    
    
    
#31
aData4_3or4_31<-subset(aData4_3or4, Temp==31)
aData4_3or4_31_D3<-subset(aData4_3or4_31, Day==3)
    aData4_3or4_31_D3_g3<-subset(aData4_3or4_31_D3, numbClades==3)
    aData4_3or4_31_D3_g4<-subset(aData4_3or4_31_D3, numbClades==4)
aData4_3or4_31_D7<-subset(aData4_3or4_31, Day==7)
    aData4_3or4_31_D7_g3<-subset(aData4_3or4_31_D7, numbClades==3)
    aData4_3or4_31_D7_g4<-subset(aData4_3or4_31_D7, numbClades==4)

#1 or 2
    aData4_1or2_31<-subset(aData4_1or2, Temp==31)
aData4_1or2_31_D3<-subset(aData4_1or2_31, Day==3)
    aData4_1or2_31_D3_g1<-subset(aData4_1or2_31_D3, numbClades==1)
    aData4_1or2_31_D3_g2<-subset(aData4_1or2_31_D3, numbClades==2)
aData4_1or2_31_D7<-subset(aData4_1or2_31, Day==7)
    aData4_1or2_31_D7_g1<-subset(aData4_1or2_31_D7, numbClades==1)
    aData4_1or2_31_D7_g2<-subset(aData4_1or2_31_D7, numbClades==2)
    

#testing stacked bar - INDIVIDUAL LARVAE

#27C 

#Day3 3gen
aData4_3or4_27_D3_g3_plot<-ggplot(aData4_3or4_27_D3_g3, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
           geom_bar(stat="identity", width=.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("27C_D3_3gen");aData4_3or4_27_D3_g3_plot

#day 7 3gen
aData4_3or4_27_D7_g3_plot<-ggplot(aData4_3or4_27_D7_g3, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
           geom_bar(stat="identity", width=0.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("27C_D7_3gen");aData4_3or4_27_D7_g3_plot

#day 14 3 gen
aData4_3or4_27_D14_g3_plot<-ggplot(aData4_3or4_27_D14_g3, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) +
           geom_bar(stat="identity", width=0.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("27C_D14_3gen");aData4_3or4_27_D3_g3_plot

aData4_3or4_27_D3_g3_plot<-aData4_3or4_27_D3_g3_plot+ theme(legend.position = "none") #remove legend
aData4_3or4_27_D7_g3_plot<-aData4_3or4_27_D7_g3_plot+ theme(legend.position = "none") #remove legend
aData4_3or4_27_D14_g3_plot<-aData4_3or4_27_D14_g3_plot+ theme(legend.position = "none") #remove legend

grid.arrange(aData4_3or4_27_D3_g3_plot, aData4_3or4_27_D7_g3_plot,aData4_3or4_27_D14_g3_plot, nrow = 1)


#4 genera

#D3 4gen
aData4_3or4_27_D3_g4_plot<-ggplot(aData4_3or4_27_D3_g4, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
           geom_bar(stat="identity", width=.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("27C_D3_4gen");aData4_3or4_27_D3_g4_plot

#day7  4gen
aData4_3or4_27_D7_g4_plot<-ggplot(aData4_3or4_27_D7_g4, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
           geom_bar(stat="identity", width=0.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("27C_D7_4gen");aData4_3or4_27_D7_g4_plot

#day 14 4gen
aData4_3or4_27_D14_g4_plot<-ggplot(aData4_3or4_27_D14_g4, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
           geom_bar(stat="identity", width=0.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("27C_D14_4gen");aData4_3or4_27_D14_g4_plot

aData4_3or4_27_D3_g4_plot<-aData4_3or4_27_D3_g4_plot+ theme(legend.position = "none") #remove legend
aData4_3or4_27_D7_g4_plot<-aData4_3or4_27_D7_g4_plot+ theme(legend.position = "none") #remove legend
aData4_3or4_27_D14_g4_plot<-aData4_3or4_27_D14_g4_plot+ theme(legend.position = "none") #remove legend

grid.arrange(aData4_3or4_27_D3_g3_plot, aData4_3or4_27_D7_g3_plot,aData4_3or4_27_D14_g3_plot,aData4_3or4_27_D3_g4_plot, aData4_3or4_27_D7_g4_plot,aData4_3or4_27_D14_g4_plot, nrow = 2)
...
#3gen
#D3
aData4_3or4_30_D3_g3_plot<-ggplot(aData4_3or4_30_D3_g3, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
           geom_bar(stat="identity", width=.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("30C_D3_3gen");aData4_3or4_30_D3_g3_plot

aData4_3or4_30_D7_g3_plot<-ggplot(aData4_3or4_30_D7_g3, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
           geom_bar(stat="identity", width=0.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("30C_D7_3gen");aData4_3or4_30_D7_g3_plot

aData4_3or4_30_D14_g3_plot<-ggplot(aData4_3or4_30_D14_g3, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
           geom_bar(stat="identity", width=0.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("30C_D14_3gen");aData4_3or4_30_D14_g3_plot

aData4_3or4_30_D3_g3_plot<-aData4_3or4_30_D3_g3_plot+ theme(legend.position = "none") #remove legend
aData4_3or4_30_D7_g3_plot<-aData4_3or4_30_D7_g3_plot+ theme(legend.position = "none") #remove legend
aData4_3or4_30_D14_g3_plot<-aData4_3or4_30_D14_g3_plot+ theme(legend.position = "none") #remove legend

grid.arrange(aData4_3or4_30_D3_g3_plot, aData4_3or4_30_D7_g3_plot,aData4_3or4_30_D14_g3_plot, nrow = 1)


#4 genera

#D3
aData4_3or4_30_D3_g4_plot<-ggplot(aData4_3or4_30_D3_g4, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
           geom_bar(stat="identity", width=.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("30C_D3_4gen");aData4_3or4_30_D3_g4_plot

aData4_3or4_30_D7_g4_plot<-ggplot(aData4_3or4_30_D7_g4, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
           geom_bar(stat="identity", width=0.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("30C_D7_4gen");aData4_3or4_30_D7_g4_plot

aData4_3or4_30_D14_g4_plot<-ggplot(aData4_3or4_30_D14_g4, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
           geom_bar(stat="identity", width=0.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("30C_D14_4gen");aData4_3or4_30_D3_g4_plot

aData4_3or4_30_D3_g4_plot<-aData4_3or4_30_D3_g4_plot+ theme(legend.position = "none") #remove legend
aData4_3or4_30_D7_g4_plot<-aData4_3or4_30_D7_g4_plot+ theme(legend.position = "none") #remove legend
aData4_3or4_30_D14_g4_plot<-aData4_3or4_30_D14_g4_plot+ theme(legend.position = "none") #remove legend

grid.arrange(aData4_3or4_30_D3_g3_plot, aData4_3or4_30_D7_g3_plot,aData4_3or4_30_D14_g3_plot,aData4_3or4_30_D3_g4_plot, aData4_3or4_30_D7_g4_plot,aData4_3or4_30_D14_g4_plot, nrow = 2)
...
#3gen
#D3
aData4_3or4_31_D3_g3_plot<-ggplot(aData4_3or4_31_D3_g3, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
           geom_bar(stat="identity", width=.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("31C_D3_3gen");aData4_3or4_31_D3_g3_plot

aData4_3or4_31_D7_g3_plot<-ggplot(aData4_3or4_31_D7_g3, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
           geom_bar(stat="identity", width=0.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("31C_D7_3gen");aData4_3or4_31_D7_g3_plot



aData4_3or4_31_D3_g3_plot<-aData4_3or4_31_D3_g3_plot+ theme(legend.position = "none") #remove legend
aData4_3or4_31_D7_g3_plot<-aData4_3or4_31_D7_g3_plot+ theme(legend.position = "none") #remove legend

 grid.arrange(aData4_3or4_31_D3_g3_plot, aData4_3or4_31_D7_g3_plot, nrow = 1)


#4 genera

#D3
aData4_3or4_31_D3_g4_plot<-ggplot(aData4_3or4_31_D3_g4, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
           geom_bar(stat="identity", width=.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("31C_D3_4gen");aData4_3or4_31_D3_g4_plot

aData4_3or4_31_D7_g4_plot<-ggplot(aData4_3or4_31_D7_g4, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
           geom_bar(stat="identity", width=0.8)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
            #scale_fill_locuszoom()+
   xlab("Larva")+
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("31C_D7_4gen");aData4_3or4_31_D7_g4_plot



aData4_3or4_31_D3_g4_plot<-aData4_3or4_31_D3_g4_plot+ theme(legend.position = "none") #remove legend
aData4_3or4_31_D7_g4_plot<-aData4_3or4_31_D7_g4_plot+ theme(legend.position = "none") #remove legend

grid.arrange(aData4_3or4_31_D3_g3_plot, aData4_3or4_31_D7_g3_plot,aData4_3or4_31_D3_g4_plot, aData4_3or4_31_D7_g4_plot, nrow = 2)
...
# C Who was the most dominant clade in larvave. Only larvae where all four genera amped
...
domAll_31_plot<-domAll_31_plot+ theme(legend.position = "none") #remove legend
...
```{r}     
 ...
 #not sure this is correct. it's 0s for 3 losers and 1 for the winner per larva
...
#write.csv(ALL_RelA_b,"ALL_RelA_b.csv")
``` 
...
ALL_RelA_b_mod<-lmer(cells.larvae_log~Symbiont*Day*Temp+(1|ALL.number)+(1|Pot), ALL_RelA_b)
anova(ALL_RelA_b_mod)
qqPlot(residuals(ALL_RelA_b_mod))   # good

  emm<-emmeans(ALL_RelA_b_mod,  ~ Symbiont *Day*Temp, adjust="Tukey")
cld(emm)
pairs(emm)

  emm<-emmeans(ALL_RelA_b_mod,  ~ Day*Temp|Symbiont, adjust="Tukey")
cld(emm)
pairs(emm)


##Subset 27 and 30C
ALL_RelA_b_27C30C<-subset(ALL_RelA_b, Temp=="27"|Temp=="30")

ALL_RelA_b_27C30C_mod<-lmer(cells.larvae_log~Symbiont*Day*Temp+(1|ALL.number)+(1|Pot), ALL_RelA_b_27C30C)
anova(ALL_RelA_b_27C30C_mod)
qqPlot(residuals(ALL_RelA_b_27C30C_mod))   # meh

  emm<-emmeans(ALL_RelA_b_27C30C_mod,  ~ Symbiont *Day*Temp, adjust="Tukey")
cld(emm)
pairs(emm)

##Subset Day 3 and D7
ALL_RelA_b_D3D7<-subset(ALL_RelA_b, Day=="3"|Day=="7")

ALL_RelA_b_D3D7_mod<-lmer(cells.larvae_log~Symbiont*Temp*Day+(1|ALL.number)+(1|Pot), ALL_RelA_b_D3D7)
anova(ALL_RelA_b_D3D7_mod)
qqPlot(residuals(ALL_RelA_b_D3D7_mod))   # meh

  emm<-emmeans(ALL_RelA_b_D3_mod,  ~ Symbiont |Temp, adjust="Tukey")
cld(emm)
pairs(emm)
...
By temps - weird boxplot chart good AND BAR chart (used)
```{r}    
...
# Table H: NEW Plot cells/larvae vs number of clades infected - shows that as more cells infected more clades infected
```{r}
...
#merge 2 dfs
CellsByNum<-merge(ALL_RelA_Infect2,ALLcast_9000_cast4, by="ALL.number")
...
#plot 27 
CellsByNum_plot27<-ggplot(data=CellsByNum_27C, aes(x=cells.larvae_log, y=numclade, color=Day)) +
  #geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
             #   width=0.1, show.legend = T)+
  geom_point( size=4, show.legend = T)+ 
      scale_color_manual(values=c('#D43F3AFF','#EEA236FF','#5CB85CFF','#46B8DAFF'))+
...
#plot 30 
CellsByNum_plot30<-ggplot(data=CellsByNum_30C, aes(x=cells.larvae_log, y=numclade, color=Day)) +
  #geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
             #   width=0.1, show.legend = T)+
  geom_point( size=4, show.legend = T)+ 
      scale_color_manual(values=c('#D43F3AFF','#EEA236FF','#5CB85CFF','#46B8DAFF'))+
...
#plot 31
CellsByNum_plot31<-ggplot(data=CellsByNum_31C, aes(x=cells.larvae_log, y=numclade, color=Day)) +
  #geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
             #   width=0.1, show.legend = T)+
  geom_point( size=4, show.legend = T)+ 
      scale_color_manual(values=c('#D43F3AFF','#EEA236FF','#5CB85CFF','#46B8DAFF'))+
...
# 78 larvae out of 87 larvae total are infected wit 3 or 4 clades, 90%
...
qqPlot(residuals(CellsByNum_model))   # good
  ...
  emm<-emmeans(CellsByNum_model,  ~ numclade, adjust="Tukey")
cld(emm)
pairs(emm)

#if numclade is factor  YES??????
...
#Competition-Infection: Models, full and by-clade for comparison (you said you don't want to use for main analysis)
```{r}   
...
#POLYCULTURE data, ALL1 data (cutoff 9000)   
...
Things you want to remove

#richness test Aug 14, 2020 CN chat
```{r}  
ALL_RelA_Infect2<-ALL_RelA_Infect #make a copy
ALL_RelA_Infect2<-ALL_RelA_Infect2[c(3,9)] #keep cols you want
Competition3<-Competition[c(1:4,10)]  #keep cols you want (Competition df is made below so might not knit)

#bind
richnessBYcount<-merge(ALL_RelA_Infect2,Competition3, by="ALL.number")

richnessBYcount$log<-log10(richnessBYcount$totalcells+1)
richnessBYcount2 <- richnessBYcount[apply(richnessBYcount!=0, 1, all),]

plot(richnessBYcount2$log, richnessBYcount2$numclade)
plot(richnessBYcount$log, richnessBYcount$numclade)

#richness.per.larva.as.factor~logcount.zoox.per.larva

richnessBYcount2$numclade<-as.factor(as.character(richnessBYcount2$numclade))
fhfhf<-lm(numclade~log,richnessBYcount2)

#write.csv(richnessBYcount2,"richnessBYcount2.csv")

#Richness~time*temp + (1|logcount)
richmod1<-lmer(numclade~Day*Temp+(1|log),richnessBYcount2)
an



Competition4<-Competition[c(2:3, 5:8)]
Competition4$C1log<-log10(Competition4$C1+1)
Competition4$D1log<-log10(Competition4$D1+1)
Competition4$F1log<-log10(Competition4$F1+1)
Competition4$G3log<-log10(Competition4$G3+1)

Competition5<-Competition4[c(1:2, 7:10)]
Competition6<-melt(Competition5)
names(Competition6)[names(Competition6)=="value"] <- "logcount"
names(Competition6)[names(Competition6)=="variable"] <- "symbiont"

#count~sym*time*temp+(1|larva)
Richmod<-lm(logcount~symbiont*Day*Temp, Competition6)
anova(Richmod)
summary(Richmod)



# Richness~time*temp + (1|logcount)  remove 0s
richnessBYcount_No0<-
```
...
THINGS TO DELETE ONCE YOU ARE SURE
...
REMOVE WHEN YOU ARE SURE

#REMOVESingle-genus Infection: Plots: by clade,cell counts (count data, with 0s included) 
...
#REMOVE Single-genus infection: Models, log transformed
Model: `lmer(Infect~Symbiont*Temp*Day+(1|PotCode))`
using log transformed! (Cells.larvae_log)  
Single infections (histograms and full model) 
```{r}      
...
Models by temp to compare genera:  
27C
```{r} 
#27C    
...
#A. Competition infection: Binomial (GLMER). Did larvae infect or not, by clade and across clades. 
Plots and comparison models singe v multi
THIS IS WITH ONLY LARVAE WHERE ALL 4 GENERA AMPED (see below for all amps by clade)
```{r}     
...
#Monoculture & Polyculture-infection: Models by symbiont (0s included still)
```{r}   
#stats, log10
...
#HERE C1 AND F1 ARE NOT AMAZING....
CountsCompare_C1<-subset(CountsCompare, Symbiont=="C1")
hist(CountsCompare_C1$cells.larvae_round) #
CountsCompare_C1$cells.larvae_log<-log10(CountsCompare_C1$cells.larvae_round+1)
hist(CountsCompare_C1$cells.larvae_log)


CountsCompare_D1<-subset(CountsCompare, Symbiont=="D1")
hist(CountsCompare_D1$cells.larvae_round) #
CountsCompare_D1$cells.larvae_log<-log10(CountsCompare_D1$cells.larvae_round+1)
hist(CountsCompare_D1$cells.larvae_log)


CountsCompare_F1<-subset(CountsCompare, Symbiont=="F1")
hist(CountsCompare_F1$cells.larvae_round) #
CountsCompare_F1$cells.larvae_log<-log10(CountsCompare_F1$cells.larvae_round+1)
hist(CountsCompare_F1$cells.larvae_log)

CountsCompare_G3<-subset(CountsCompare, Symbiont=="G3")
hist(CountsCompare_G3$cells.larvae_round) #
CountsCompare_G3$cells.larvae_log<-log10(CountsCompare_G3$cells.larvae_round+1)
hist(CountsCompare_G3$cells.larvae_log)


#Full model single v multi <- no reason to do this
...
qqPlot(residuals(CountsCompare_C1_mod))   # good
  emm<-emmeans(CountsCompare_C1_mod,  ~ single.multi|Temp|Day, adjust="Tukey") 
cld(emm)
 
#using counts (not transformed)
CountsCompare_C1_mod2<-lmer(cells.larvae_round~Temp*Day*single.multi+(1|PotCode),CountsCompare_C1)
anova(CountsCompare_C1_mod2)

model.sel(CountsCompare_C1_mod, CountsCompare_C1_mod2)
...
qqPlot(residuals(CountsCompare_D1_mod))   # ok
   emm<-emmeans(CountsCompare_D1_mod,  ~ single.multi|Temp|Day, adjust="Tukey") 
cld(emm)

CountsCompare_D1_mod2<-lmer(cells.larvae_round~Temp*Day*single.multi+(1|PotCode),CountsCompare_D1)
anova(CountsCompare_D1_mod2)

model.sel(CountsCompare_D1_mod, CountsCompare_D1_mod2)
...
qqPlot(residuals(CountsCompare_F1_mod))   # ok
 emm<-emmeans(CountsCompare_F1_mod,  ~ single.multi|Temp|Day, adjust="Tukey") 
cld(emm)
 emm<-emmeans(CountsCompare_G3_mod,  ~ single.multi|Temp|Day, adjust="Tukey") 
cld(emm)

CountsCompare_F1_mod2<-lmer(cells.larvae_round~Temp*Day*single.multi+(1|PotCode),CountsCompare_F1)
anova(CountsCompare_F1_mod2)

model.sel(CountsCompare_F1_mod, CountsCompare_F1_mod2)
...
CountsCompare_G3_mod2<-lmer(cells.larvae_round~Temp*Day*single.multi+(1|PotCode),CountsCompare_G3)
anova(CountsCompare_G3_mod2)

model.sel(CountsCompare_G3_mod, CountsCompare_G3_mod2)
...
#Single-genus & Competition: Binomial infection PLOTS by clade (pink and blue)
...
C1_binary_SinMul_mean_31<-subset(C1_binary_SinMul_mean, Temp=="31")
 C1_binary_SinMul_mean_31<-subset(C1_binary_SinMul_mean_31, Day=="3" |Day=="7")
...
F1_binary_SinMul_mean_31<-subset(F1_binary_SinMul_mean, Temp=="31")
 F1_binary_SinMul_mean<-subset(F1_binary_SinMul_mean, Day=="3" |Day=="7")
...
#31
F1_binary_SinMul_plot_31<-ggplot(data=F1_binary_SinMul_mean_31, aes(x=Day, y=mean, group=infectType, color=infectType)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.4, show.legend = T)+
  geom_point(aes(color=infectType), size=4, show.legend = T)+
  geom_line(aes(color=infectType, linetype=infectType))+
...
G3_binary_SinMul_mean_31<-subset(G3_binary_SinMul_mean, Temp=="31")
 G3_binary_SinMul_mean_31<-subset(G3_binary_SinMul_mean_31, Day=="3" |Day=="7")
...
#save
ggsave(filename="Figures/BinaryInfect_FigbNEW3.pdf", plot=BinaryInfect_Fig, dpi=300, width=12, height=17, units="in") 
``` 




#MOno vs poly:with all poly data not subset rela Binomial infection PLOTS by clade (pink and blue)
Models Singe vs Multi by clade <- probably want to not do this, and do FULL models instead
```{r} 
  
#models
#C1
#27 
C1_binaryb_SinMul_27<-subset(C1_binaryb_SinMul, Temp==27)
 C1_binaryb_SinMul_27_mod<-glmer(Infect~Day*infectType+(1|PotCode), family="binomial", C1_binaryb_SinMul_27)
  Anova(C1_binaryb_SinMul_27_mod)
  dispersion_glmer(C1_binaryb_SinMul_27_mod)  #no overdispersion 
  emm<-emmeans(C1_binaryb_SinMul_27_mod,  ~ infectType|Day, adjust="Tukey")
cld(emm)

C1_binaryb_SinMul_30<-subset(C1_binaryb_SinMul, Temp==30)
 C1_binaryb_SinMul_30_mod<-glmer(Infect~Day*infectType+(1|PotCode), family="binomial", C1_binaryb_SinMul_30)
  Anova(C1_binaryb_SinMul_30_mod)
  dispersion_glmer(C1_binaryb_SinMul_30_mod)  #no overdispersion 
  emm<-emmeans(C1_binaryb_SinMul_30_mod,  ~ infectType|Day, adjust="Tukey")
cld(emm)

C1_binaryb_SinMul_31<-subset(C1_binaryb_SinMul, Temp==31)
 C1_binaryb_SinMul_31_mod<-glmer(Infect~Day*infectType+(1|PotCode), family="binomial", C1_binaryb_SinMul_31)
  Anova(C1_binaryb_SinMul_31_mod)
  dispersion_glmer(C1_binaryb_SinMul_31_mod)  #no overdispersion 
  emm<-emmeans(C1_binaryb_SinMul_31_mod,  ~ infectType|Day, adjust="Tukey")
cld(emm)


#D1
#27
D1_binaryb_SinMul_27<-subset(D1_binaryb_SinMul, Temp==27)
 D1_binaryb_SinMul_27_mod<-glmer(Infect~Day*infectType+(1|PotCode), family="binomial", D1_binaryb_SinMul_27)
  Anova(D1_binaryb_SinMul_27_mod)
  dispersion_glmer(D1_binaryb_SinMul_27_mod)  #no overdispersion 
  emm<-emmeans(D1_binaryb_SinMul_27_mod,  ~ infectType|Day, adjust="Tukey")
cld(emm)

D1_binaryb_SinMul_30<-subset(D1_binaryb_SinMul, Temp==30)
 D1_binaryb_SinMul_30_mod<-glmer(Infect~Day*infectType+(1|PotCode), family="binomial", D1_binaryb_SinMul_30)
  Anova(D1_binaryb_SinMul_30_mod)
  dispersion_glmer(D1_binaryb_SinMul_30_mod)  #no overdispersion 
  emm<-emmeans(D1_binaryb_SinMul_30_mod,  ~ infectType|Day, adjust="Tukey")
cld(emm)

D1_binaryb_SinMul_31<-subset(D1_binaryb_SinMul, Temp==31)
D1_binaryb_SinMul_31<-subset(D1_binaryb_SinMul_31, Day==3|Day==7)
 D1_binaryb_SinMul_31_mod<-glmer(Infect~Day*infectType+(1|PotCode), family="binomial", D1_binaryb_SinMul_31)
  Anova(D1_binaryb_SinMul_31_mod)
  dispersion_glmer(D1_binaryb_SinMul_31_mod)  #no overdispersion 
  emm<-emmeans(D1_binaryb_SinMul_31_mod,  ~ infectType|Day, adjust="Tukey")
cld(emm)

#F1
#27
F1_binaryb_SinMul_27<-subset(F1_binaryb_SinMul, Temp==27)
 F1_binaryb_SinMul_27_mod<-glmer(Infect~Day*infectType+(1|PotCode), family="binomial", F1_binaryb_SinMul_27)
  Anova(F1_binaryb_SinMul_27_mod)
  dispersion_glmer(F1_binaryb_SinMul_27_mod)  #no overdispersion 
  emm<-emmeans(F1_binaryb_SinMul_27_mod,  ~ infectType|Day, adjust="Tukey")
cld(emm)

F1_binaryb_SinMul_30<-subset(F1_binaryb_SinMul, Temp==30)
 F1_binaryb_SinMul_30_mod<-glmer(Infect~Day*infectType+(1|PotCode), family="binomial", F1_binaryb_SinMul_30)
  Anova(F1_binaryb_SinMul_30_mod)
  dispersion_glmer(F1_binaryb_SinMul_30_mod)  #no overdispersion 
  emm<-emmeans(F1_binaryb_SinMul_30_mod,  ~ infectType|Day, adjust="Tukey")
cld(emm)

F1_binaryb_SinMul_31<-subset(F1_binaryb_SinMul, Temp==31)
 F1_binaryb_SinMul_31_mod<-glmer(Infect~Day*infectType+(1|PotCode), family="binomial", F1_binaryb_SinMul_31)
  Anova(F1_binaryb_SinMul_31_mod)
  dispersion_glmer(F1_binaryb_SinMul_31_mod)  #no overdispersion 
  emm<-emmeans(F1_binaryb_SinMul_31_mod,  ~ infectType|Day, adjust="Tukey")
cld(emm)

#G3
#27
G3_binaryb_SinMul_27<-subset(G3_binaryb_SinMul, Temp==27)
 G3_binaryb_SinMul_27_mod<-glmer(Infect~Day*infectType+(1|PotCode), family="binomial", G3_binaryb_SinMul_27)
  Anova(G3_binaryb_SinMul_27_mod)
  dispersion_glmer(G3_binaryb_SinMul_27_mod)  #no overdispersion 
  emm<-emmeans(G3_binaryb_SinMul_27_mod,  ~ infectType|Day, adjust="Tukey")
cld(emm)

G3_binaryb_SinMul_30<-subset(G3_binaryb_SinMul, Temp==30)
 G3_binaryb_SinMul_30_mod<-glmer(Infect~Day*infectType+(1|PotCode), family="binomial", G3_binaryb_SinMul_30)
  Anova(G3_binaryb_SinMul_30_mod)
  dispersion_glmer(G3_binaryb_SinMul_30_mod)  #no overdispersion 
  emm<-emmeans(G3_binaryb_SinMul_30_mod,  ~ infectType|Day, adjust="Tukey")
cld(emm)

G3_binaryb_SinMul_31<-subset(G3_binaryb_SinMul, Temp==31)
 G3_binaryb_SinMul_31_mod<-glmer(Infect~Day*infectType+(1|PotCode), family="binomial", G3_binaryb_SinMul_31)
  Anova(G3_binaryb_SinMul_31_mod)
  dispersion_glmer(G3_binaryb_SinMul_31_mod)  #no overdispersion 
  emm<-emmeans(G3_binaryb_SinMul_31_mod,  ~ infectType|Day, adjust="Tukey")
cld(emm)
...
#Monoculture Infection: Fig 1: Heat maps, by clade (temp x time)  
   library(gplots)
...
col_breaks = c(seq(0,1,length=100),  # for red
  seq(1.01,2.0,length=100),           # for yellow
  seq(2.1,3,length=100))             # for green
