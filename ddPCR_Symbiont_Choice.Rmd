...
###Not working yet. 
#### try using Mono-poly as an effect with nested genera
Mort.df_NEW_MonPol<-Mort.df_NEW
Mort.df_NEW_MonPol$MonoPoly<-Mort.df_NEW_MonPol$Symbiont
#create monoy poly category
Mort.df_NEW_MonPol$MonoPoly<-as.character(Mort.df_NEW_MonPol$MonoPoly)
Mort.df_NEW_MonPol$MonoPoly[Mort.df_NEW_MonPol$MonoPoly=="C1"] <- "Mono"
Mort.df_NEW_MonPol$MonoPoly[Mort.df_NEW_MonPol$MonoPoly=="D1"] <- "Mono"
Mort.df_NEW_MonPol$MonoPoly[Mort.df_NEW_MonPol$MonoPoly=="F1"] <- "Mono"
Mort.df_NEW_MonPol$MonoPoly[Mort.df_NEW_MonPol$MonoPoly=="G3"] <- "Mono"
Mort.df_NEW_MonPol$MonoPoly[Mort.df_NEW_MonPol$MonoPoly=="All"] <- "Poly"
Mort.df_NEW_MonPol$MonoPoly<-as.factor(as.character(Mort.df_NEW_MonPol$MonoPoly))
#full model
##create binomial vector
attach(Mort.df_NEW_MonPol)
x<-cbind(Alive, Failures)

#By mono-poly
Mort.df_NEW_MonPol_mod<-glmer(x~Temp*Day*Symbiont + (1+Symbiont)+(1|Pot), family=binomial)
summary(Mort.df_NEW_MonPol_mod)
Anova(Mort.df_NEW_MonPol_mod)
 emm<-emmeans(Mort.df_NEW_MonPol_mod,  ~Temp*Day|Symbiont, adjust="Tukey")
cld(emm, Letter="abcdefg")
pairs(emm)

detach(Mort.df_NEW_MonPol)
...
Poly_no0_27_mean_plot<-ggplot(data=Poly_no0_27_mean, aes(x=Day.num, y=mean, group = Symbiont, color=Symbiont)) +
  ...
  #ylim(0,3200)+
       scale_x_continuous(breaks=seq(0, 14, 2), limits =c(0,14))+
       scale_y_log10(breaks = c(0,100,1000, 10000), limits=c(.1,5000))+
  theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        axis.title=element_text(size=14,face="bold"), #Set axis format
        panel.border = element_blank(), #Set the border 
...
#due to low n's at 31C day 7 and 14, we need to remove
...
##########@AH: Is this the best way to set up the mean table. I turn it into relative abundance, and then for each jar, I get the means per infection group (0,1,2,3,4). But for some reason, I'm just not sure this is right for the Mean and s.e. 
...
#Kruskal test - try by creating day-temp code and by running by day
  ...
  #by  temp and day <- USE THIS, you could do Dunn test. but prob dont need it. 
...
NumClades_stats_30<-subset(NumClades_stats, Temp==30)
    kruskal.test(numclade~Day, data=NumClades_stats_30)
     DT = dunnTest(numclade~Day,
              data=NumClades_stats_30,
...
NumClades_stats_D3<-subset(NumClades_stats, Day==3)
    ...
    #Dunn test Post hoc   
    DT = dunnTest(numclade~Temp,
              data=NumClades_stats_D3,
              method="bh")      # Adjusts p-values for multiple comparisons;
                                # See ?dunnTest for options
#day 7    
...
Now lets do relA for each grouping of number of genera/larva  - this may be unnecessary
df: ALL_RelA_binaryb (relative abundance, the cells.larvae=Rel abund)
```{r}     
aData<-RelA_binaryb
aData<-na.omit(aData) #remove larae with values not present for all 4 genera

#  aData$C1[aData$Cladocopium>0]<-1  #make new cols for infect or not 1 for infect, 0 not.
...
aData3<-merge(aData2,ALL_RelAb_b, by="ALL.number")
...
NumWinner_27RelA_D3a<-NumWinner_27RelA_D3[c("numclade","Symbiont","cells.larvae")] #keep only cols you need, btw cells. larvae is actually relative abundance
NumWinner_27RelA_D3a_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_27RelA_D3a, mean) #aggregate by symbiont + numclade

  #27 Day 3 by 3gen  <- use to make new spreadsheet with relA for 3-gen and 4-gen
  NumWinner_27RelA_D3b<-NumWinner_27RelA_D3[c("numclade","Symbiont","cells.larvae")]
  NumWinner_27RelA_D3b_3gen<-subset(NumWinner_27RelA_D3b, numclade=="3")
  NumWinner_27RelA_D3b_3gen_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_27RelA_D3b_3gen, mean)
  NumWinner_27RelA_D3b_3gen$Day<-3
    NumWinner_27RelA_D3b_3gen$Temp<-27
    NumWinner_27RelA_D3b_3gen$genera<-3

   #27 Day 3 by 4gen
  NumWinner_27RelA_D3b_4gen<-subset(NumWinner_27RelA_D3b, numclade=="4")
  NumWinner_27RelA_D3b_4gen_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_27RelA_D3b_4gen, mean)
...
NumWinner_27RelA_D7a<-NumWinner_27RelA_D7[c("numclade","Symbiont","cells.larvae")] #keep only cols you need
NumWinner_27RelA_D7a_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_27RelA_D7a, mean) #aggregate by symbiont + numclade

 #27 Day 7 by 3 
  NumWinner_27RelA_D7b<-NumWinner_27RelA_D7[c("numclade","Symbiont","cells.larvae")]
  NumWinner_27RelA_D7b_3gen<-subset(NumWinner_27RelA_D7b, numclade=="3")
  NumWinner_27RelA_D7b_3gen_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_27RelA_D7b_3gen, mean)
  
   #27 Day 7 by 4
  NumWinner_27RelA_D7b_4gen<-subset(NumWinner_27RelA_D7b, numclade=="4")
  NumWinner_27RelA_D7b_4gen_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_27RelA_D7b_4gen, mean)

   #27 Day 7 by 2 genera 
  NumWinner_27RelA_D7b_2gen<-subset(NumWinner_27RelA_D7b, numclade=="2")
  NumWinner_27RelA_D7b_2gen_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_27RelA_D7b_2gen, mean)


#D14
NumWinner_27RelA_D14<-subset(NumWinner_27RelA, Day==14) #subset day 14
NumWinner_27RelA_D14a<-NumWinner_27RelA_D14[c("numclade","Symbiont","cells.larvae")] #keep only cols you need
NumWinner_27RelA_D14a_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_27RelA_D14a, mean) #aggregate by symbiont + numclade

 #27 Day 14 by 3 
  NumWinner_27RelA_D14b<-NumWinner_27RelA_D14[c("numclade","Symbiont","cells.larvae")]
  NumWinner_27RelA_D14b_3gen<-subset(NumWinner_27RelA_D14b, numclade=="3")
  NumWinner_27RelA_D14b_3gen_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_27RelA_D14b_3gen, mean)
  
   #27 Day 14 by 4
  NumWinner_27RelA_D14b_4gen<-subset(NumWinner_27RelA_D14b, numclade=="4")
  NumWinner_27RelA_D14b_4gen_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_27RelA_D14b_4gen, mean)
...
# barplot 27 D3 
NumWinner_27RelA_D3a_agg_plot<-ggplot(NumWinner_27RelA_D3a_agg, aes(y=cells.larvae,x=numclade, fill=Symbiont)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of Genera")+
...
# barplot 27 D7 
NumWinner_27RelA_D7_agg_plot<-ggplot(NumWinner_27RelA_D7a_agg, aes(y=cells.larvae,x=numclade, fill=Symbiont)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
...
# barplot 27 D14
NumWinner_27RelA_D14_agg_plot<-ggplot(NumWinner_27RelA_D14a_agg, aes(y=cells.larvae,x=numclade, fill=Symbiont)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of General")+
...
NumWinner_30RelA_D3a<-NumWinner_30RelA_D3[c("numclade","Symbiont","cells.larvae")] #keep only cols you need
NumWinner_30RelA_D3a_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_30RelA_D3a, mean) #aggregate by symbiont + numclade

#30 Day 3 by 3  <- use to make new spreadsheet with relA for 3-gen and 4-gen
  NumWinner_30RelA_D3b<-NumWinner_30RelA_D3[c("numclade","Symbiont","cells.larvae")]
  NumWinner_30RelA_D3b_3gen<-subset(NumWinner_30RelA_D3b, numclade=="3")
  NumWinner_30RelA_D3b_3gen_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_30RelA_D3b_3gen, mean)
  
   #30 Day 3 by 4
  NumWinner_30RelA_D3b_4gen<-subset(NumWinner_30RelA_D3b, numclade=="4")
  NumWinner_30RelA_D3b_4gen_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_30RelA_D3b_4gen, mean)


#D7
NumWinner_30RelA_D7<-subset(NumWinner_30RelA, Day==7) #subset day 7
NumWinner_30RelA_D7a<-NumWinner_30RelA_D7[c("numclade","Symbiont","cells.larvae")] #keep only cols you need
NumWinner_30RelA_D7a_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_30RelA_D7a, mean) #aggregate by symbiont + numclade

#30 Day 7 by 3 
  NumWinner_30RelA_D7b<-NumWinner_30RelA_D7[c("numclade","Symbiont","cells.larvae")]
  NumWinner_30RelA_D7b_3gen<-subset(NumWinner_30RelA_D7b, numclade=="3")
  NumWinner_30RelA_D7b_3gen_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_30RelA_D7b_3gen, mean)
  
   #30 Day 7 by 4
  NumWinner_30RelA_D7b_4gen<-subset(NumWinner_30RelA_D7b, numclade=="4")
  NumWinner_30RelA_D7b_4gen_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_30RelA_D7b_4gen, mean)



#D14
NumWinner_30RelA_D14<-subset(NumWinner_30RelA, Day==14) #subset day 14
NumWinner_30RelA_D14a<-NumWinner_30RelA_D14[c("numclade","Symbiont","cells.larvae")] #keep only cols you need
NumWinner_30RelA_D14a_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_30RelA_D14a, mean) #aggregate by symbiont + numclade


#30 Day 14 by 3 
  NumWinner_30RelA_D14b<-NumWinner_30RelA_D14[c("numclade","Symbiont","cells.larvae")]
  NumWinner_30RelA_D14b_3gen<-subset(NumWinner_30RelA_D14b, numclade=="3")
  NumWinner_30RelA_D14b_3gen_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_30RelA_D14b_3gen, mean)
  
   #30 Day 14 by 4
  NumWinner_30RelA_D14b_4gen<-subset(NumWinner_30RelA_D14b, numclade=="4")
  NumWinner_30RelA_D14b_4gen_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_30RelA_D14b_4gen, mean)



#Plots
# barplot 30 D3 
NumWinner_30RelA_D3a_agg_plot<-ggplot(NumWinner_30RelA_D3a_agg, aes(y=cells.larvae,x=numclade, fill=Symbiont)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of General")+
...
# barplot 30 D7 
NumWinner_30RelA_D7_agg_plot<-ggplot(NumWinner_30RelA_D7a_agg, aes(y=cells.larvae,x=numclade, fill=Symbiont)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of General")+
...
# barplot 30 D14
NumWinner_30RelA_D14_agg_plot<-ggplot(NumWinner_30RelA_D14a_agg, aes(y=cells.larvae,x=numclade, fill=Symbiont)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of General")+
...
NumWinner_31RelA_D3a<-NumWinner_31RelA_D3[c("numclade","Symbiont","cells.larvae")] #keep only cols you need
NumWinner_31RelA_D3a_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_31RelA_D3a, mean) #aggregate by symbiont + numclade

#31 Day 3 by 3  <- use to make new spreadsheet with relA for 3-gen and 4-gen
  NumWinner_31RelA_D3b<-NumWinner_31RelA_D3[c("numclade","Symbiont","cells.larvae")]
  NumWinner_31RelA_D3b_3gen<-subset(NumWinner_31RelA_D3b, numclade=="3")
  NumWinner_31RelA_D3b_3gen_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_31RelA_D3b_3gen, mean)
  
   #31 Day 3 by 4
  NumWinner_31RelA_D3b_4gen<-subset(NumWinner_31RelA_D3b, numclade=="4")
  NumWinner_31RelA_D3b_4gen_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_31RelA_D3b_4gen, mean)



#D7
NumWinner_31RelA_D7<-subset(NumWinner_31RelA, Day==7) #subset day 7
NumWinner_31RelA_D7a<-NumWinner_31RelA_D7[c("numclade","Symbiont","cells.larvae")] #keep only cols you need
NumWinner_31RelA_D7a_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_31RelA_D7a, mean) #aggregate by symbiont + numclade

  #31 Day 3 by 3  <- use to make new spreadsheet with relA for 3-gen and 4-gen
  NumWinner_31RelA_D7b<-NumWinner_31RelA_D7[c("numclade","Symbiont","cells.larvae")]
  NumWinner_31RelA_D7b_3gen<-subset(NumWinner_31RelA_D7b, numclade=="3")
  NumWinner_31RelA_D7b_3gen_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_31RelA_D7b_3gen, mean)
  
   #31 Day 3 by 4
  NumWinner_31RelA_D7b_4gen<-subset(NumWinner_31RelA_D7b, numclade=="4")
  NumWinner_31RelA_D7b_4gen_agg<-aggregate(.~Symbiont+numclade, data=NumWinner_31RelA_D7b_4gen, mean)

#Plots
# barplot 31 D3 
NumWinner_31RelA_D3a_agg_plot<-ggplot(NumWinner_31RelA_D3a_agg, aes(y=cells.larvae,x=numclade, fill=Symbiont)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of General")+
...
# barplot 31 D7 
NumWinner_31RelA_D7_agg_plot<-ggplot(NumWinner_31RelA_D7a_agg, aes(y=cells.larvae,x=numclade, fill=Symbiont)) + 
geom_bar(stat="identity", width = .6)+
  scale_fill_manual(values=c("#D43F3AFF","#EEA236FF","#5CB85CFF","#46B8DAFF"))+
       scale_x_continuous(breaks=seq(1, 4, 1), limits =c(0,5))+
   xlab("Number of General")+
...
aData4_mean <- ddply(aData4, c("Day", "Temp","numclade","Symbiont"), summarise, #summarize cell counts
                 N    = length(cells.larvae[!is.na(cells.larvae)]), #calculate the length of the data frame,  excluding NA’s *this part is important*
                 mean = mean(cells.larvae, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(cells.larvae, na.rm=TRUE), #calculate standard deviation 
                 se   = sd / sqrt(N), #calculate standard error
...
#Day 3
  aData4_27_D3<-subset(aData4_27, Day==3)
    aData4_27_D3_g3<-subset(aData4_27_D3, numclade==3)
    aData4_27_D3_g4<-subset(aData4_27_D3, numclade==4)
    aData4_27_D3_g2<-subset(aData4_27_D3, numclade==2)
    aData4_27_D3_g1<-subset(aData4_27_D3, numclade==1)
#Day 7
  aData4_27_D7<-subset(aData4_27, Day==7)
    aData4_27_D7_g3<-subset(aData4_27_D7, numclade==3)
    aData4_27_D7_g4<-subset(aData4_27_D7, numclade==4)
    aData4_27_D7_g2<-subset(aData4_27_D7, numclade==2)
    aData4_27_D7_g1<-subset(aData4_27_D7, numclade==1)
 #Day 14   
  aData4_27_D14<-subset(aData4_27, Day==14)
    aData4_27_D14_g3<-subset(aData4_27_D14, numclade==3)
    aData4_27_D14_g4<-subset(aData4_27_D14, numclade==4)
    aData4_27_D14_g2<-subset(aData4_27_D14, numclade==2)
    aData4_27_D14_g1<-subset(aData4_27_D14, numclade==1)

#30
aData4_30<-subset(aData4, Temp==30)
#Day 3
aData4_30_D3<-subset(aData4_30, Day==3)
    aData4_30_D3_g3<-subset(aData4_30_D3, numclade==3)
    aData4_30_D3_g4<-subset(aData4_30_D3, numclade==4)
    aData4_30_D3_g2<-subset(aData4_30_D3, numclade==2)
    aData4_30_D3_g1<-subset(aData4_30_D3, numclade==1)
#Day 7
aData4_30_D7<-subset(aData4_30, Day==7)
    aData4_30_D7_g3<-subset(aData4_30_D7, numclade==3)
    aData4_30_D7_g4<-subset(aData4_30_D7, numclade==4)
    aData4_30_D7_g2<-subset(aData4_30_D7, numclade==2)
    aData4_30_D7_g1<-subset(aData4_30_D7, numclade==1)
#Day 14
aData4_30_D14<-subset(aData4_30, Day==14)
    aData4_30_D14_g3<-subset(aData4_30_D14, numclade==3)
    aData4_30_D14_g4<-subset(aData4_30_D14, numclade==4)
    aData4_30_D14_g2<-subset(aData4_30_D14, numclade==2)
    aData4_30_D14_g1<-subset(aData4_30_D14, numclade==1)
    
#31
aData4_31<-subset(aData4, Temp==31)
#Dzy 3
aData4_31_D3<-subset(aData4_31, Day==3)
    aData4_31_D3_g3<-subset(aData4_31_D3, numclade==3)
    aData4_31_D3_g4<-subset(aData4_31_D3, numclade==4)
    aData4_31_D3_g2<-subset(aData4_31_D3, numclade==2)
    aData4_31_D3_g1<-subset(aData4_31_D3, numclade==1)
#Day 7    
aData4_31_D7<-subset(aData4_31, Day==7)
    aData4_31_D7_g3<-subset(aData4_31_D7, numclade==3)
    aData4_31_D7_g4<-subset(aData4_31_D7, numclade==4)
    aData4_31_D7_g2<-subset(aData4_31_D7, numclade==2)
    aData4_31_D7_g1<-subset(aData4_31_D7, numclade==1)
```

#stacked bar - INDIVIDUAL LARVAE
    #testing to plot them all in one go by temp 27
```{R}   
#27C
...
ALL_RelA2<-na.omit(ALL_RelA2) #remove larvae where not all amped
...
FIG K: Plot dominant genus/larva 
```{r}
#27C  
...
Dom30<-subset(DomAll, Temp==30)
  ...
  Anova(Dom30_mod)
    overdisp_fun(Dom30_mod)
   emm<-emmeans(Dom30_mod,  ~ clade)
  pairs(emm)
  cld(emm) 
...
#this will remove any larva with a NA for a species. This also removes a larvae with a 0 for all the species, this is because for stats, we only include infected larvae. if you want to add back manually, uncomment:
#ALL_RelAb$Cladocopium[is.nan(ALL_RelAb$Cladocopium)] <- 0
#ALL_RelAb$Durusdinium[is.nan(ALL_RelAb$Durusdinium)] <- 0
#ALL_RelAb$Fugacium[is.nan(ALL_RelAb$Fugacium)] <- 0
#ALL_RelAb$Gerakladium[is.nan(ALL_RelAb$Gerakladium)] <- 0
...
#ALL_RelAb_a_NMDS<-ALL_RelAb_a_NMDS[ALL_RelAb_a_NMDS$ALL.number != "105" & ALL_RelAb_a_NMDS$ALL.number != "134"& ALL_RelAb_a_NMDS$ALL.number != "156"& ALL_RelAb_a_NMDS$ALL.number != "195"& ALL_RelAb_a_NMDS$ALL.number != "200", ] 
...
xx = ggplot(data.scores, aes(x = NMDS1, y = NMDS2)) + 
  ...
  geom_segment(data = envfit.df, aes(x = 0, xend = NMDS1, y = 0, yend = NMDS2), arrow = arrow(length = unit(0.5, "cm")),
               colour = "grey", inherit_aes = FALSE) +
  geom_text(data = envfit.df, aes(x = NMDS1, y = NMDS2, label = rownames(envfit.df)), size = 5) +
  scale_fill_manual(values=Palette1) +  
  scale_colour_manual(values=Palette1)+
 stat_ellipse(geom = "polygon",level=.75, alpha = .2, aes(fill=Temp))+
...
#stat_ellipse(geom = "polygon",level=.25, alpha = .2, aes(fill=Day))+
 # stat_ellipse(level=.25,  aes(x=NMDS1, y=NMDS2,color=Day))
...
#lets try for each day, subset df: ALL_RelAb_a_NMDS
#D3
...
set.seed(146) #reproducible results
...
#hist ALL_RelAb_b df is right format to check
hist(ALL_RelAb_b$cells.larvae) #transform
ALL_RelAb_b$cells.larvae_arc<-asin(sqrt(ALL_RelAb_b$cells.larvae))
hist(ALL_RelAb_b$cells.larvae_arc) #beter (check to see if that is ok)
ALL_RelAb_b$cells.larvae_log<-log10(ALL_RelAb_b$cells.larvae+1)
hist(ALL_RelAb_b$cells.larvae_log) #not as good

RelA_PCA<-ALL_RelAb_a #keep only cols you need USE _a for the right shape of data (_b is melted, _a cast)
