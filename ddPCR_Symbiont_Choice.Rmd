...
p<-ggplot(data=scopehemo_ddPCRCompare,aes(x=Countmean_log, y=ddPCRmean_log)) +
  ...
  #scale_y_continuous(trans = "log10")+
theme_classic()+
 geom_abline(intercept = .18, slope = .94)+ #add in lmodel2 slope/intercept
...
predict(scopehemo_mod, newdata = new.speeds, interval = "confidence")
...
#Scope ddPCR 
scopeddPCR<- subset(scopehemo_ddPCRCompare, Day=="3")
lmodel2(ddPCRmean_log~Countmean_log, data = scopeddPCR)

#Hemo ddPCR 
hemoddPCR<- subset(scopehemo_ddPCRCompare, Day=="7"|Day=="14")
lmodel2(ddPCRmean_log~Countmean_log, data = hemoddPCR)


##### cn to dos testing adding prediction/confidence

scopehemo_mod<-lmodel2(ddPCRmean_log~Countmean_log, data = scopehemo_ddPCRCompare)
scopehemo_mod2<-lm(ddPCRmean_log~Countmean_log, data = scopehemo_ddPCRCompare)

# 1. Add predictions 
pred.int <- predict(scopehemo_mod2, interval = "prediction")
mydata <- cbind(scopehemo_ddPCRCompare, pred.int)

# 2. Regression line + confidence intervals
p <- ggplot(mydata, aes(Countmean, ddPCRmean)) +
  geom_point() +
  stat_smooth(method = lm)
