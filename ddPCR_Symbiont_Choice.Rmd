...
AH: transforming the data doesn't really do much so I dont...  
C1
```{r} 
#C1 
...
##Infection or not after 3 days - binary yes or no- INDIVIDUAL INFECTIONS
...
#C1
  ...
  Binary_C1_mod<-glmer(Infect~Temp*Day+(1|Temp/Pot), family="binomial", Binary_C1) 
  Anova(Binary_C1_mod)
  dispersion_glmer(Binary_C1_mod)  #no overdispersion 
  
#D1
  Binary_D1<-subset(BinaryInfect, Symbiont=="D1")
  Binary_D1_mod<-glmer(Infect~Temp*Day+(1|Temp/Pot), family="binomial", Binary_D1) 
  Anova(Binary_D1_mod)
...
OLD MODELS: Do you want to keep> 
```{r} 

#C1 
C1_infect<- glmer(Infect~Temp+(1|Pot),data=Initial_C1, family="binomial") #nest smallest to largest 
#C1_infect2<- glmer(Infect~Temp+(1|Pot),data=Initial_C1, family="binomial") #do this to check, same values
Anova(C1_infect)   #btw this is Type II Wald chisquare test, p=0.28
summary(C1_infect) #look at deviance of df, should be similar to df residuals. also look to ake sure it worked
summary(effect(c("Temp"), xlevels=3, C1_infect))   # THINK: you can report your means from above or the means the model has. This take into consideration the random effects so use it. 
dispersion_glmer(C1_infect) #1.14 (you want lower than 1.4)
plot(residuals(C1_infect)) + abline(h=0, lty=2) 

#D1 
D1_infect<- glmer(Infect~Temp+(1|Pot:Symbiont:Temp),data=Initial_D1, family="binomial")
Anova(D1_infect) # p=.34
summary(D1_infect)
summary(effect(c("Temp"), xlevels=3, D1_infect))   # what do these pvals coorespond to
dispersion_glmer(D1_infect) #0.829                 *****************
plot(residuals(D1_infect)) + abline(h=0, lty=2)

#F1 
F1_infect<- glmer(Infect~Temp+(1|Pot:Symbiont:Temp),data=Initial_F1, family="binomial")
Anova(F1_infect)
summary(effect(c("Temp"), xlevels=3, F1_infect))   
dispersion_glmer(F1_infect) #1.16
plot(residuals(F1_infect)) + abline(h=0, lty=2) 

#G3 
G3_infect<- glmer(Infect~Temp+(1|Pot:Symbiont:Temp),data=Initial_G3, family="binomial")
Anova(G3_infect) #p=.76
summary(effect(c("Temp"), xlevels=3, G3_infect))   # what do these pvals coorespond to
dispersion_glmer(G3_infect) #0.73                 *****************
plot(residuals(G3_infect)) + abline(h=0, lty=2) 
...
Day3_infectAll_infect<- glmer(Infect~Temp*Symbiont+(1|Pot:Symbiont:Temp),data=Day3_infectAll, family="binomial")
Anova(Day3_infectAll_infect)
summary(effect(c("Temp"), xlevels=3, Day3_infectAll_infect))   # what do these pvals coorespond to
dispersion_glmer(Day3_infectAll_infect) #0.989795                *****************
plot(residuals(Day3_infectAll_infect)) + abline(h=0, lty=2) #Dispersed randomly                      ****************

#Day 7

#C1 
Mid_C1_mod<- glmer(Infect~Temp+(1|Pot:Symbiont:Temp),data=Mid_C1, family="binomial")
...
Day7_infectAll_infect<- glmer(Infect~Temp*Symbiont+(1|Pot:Symbiont:Temp),data=Day7_infectAll, family="binomial")
Anova(Day7_infectAll_infect)
summary(Day7_infectAll) # what do these pvals coorespond to
dispersion_glmer(Day7_infectAll_infect) #0.8829768
plot(residuals(Day7_infectAll_infect)) + abline(h=0, lty=2) 

#Day 14

#C1 
Fin_C1_mod<- glmer(Infect~Temp+(1|Pot:Symbiont:Temp),data=Fin_C1, family="binomial")
Anova(Fin_C1_mod)   #btw this is Type II Wald chisquare test
summary(effect(c("Temp"), xlevels=3, Fin_C1_mod))   # what do these pvals coorespond to
dispersion_glmer(Fin_C1_mod) #.7511981
plot(residuals(Fin_C1_mod)) + abline(h=0, lty=2) #Dispersed randomly                      ****************

#D1 
Fin_D1_mod<- glmer(Infect~Temp+(1|Pot:Symbiont:Temp),data=Fin_D1, family="binomial")
Anova(Fin_D1_mod)
summary(effect(c("Temp"), xlevels=3, Fin_D1_mod))   # what do these pvals coorespond to
dispersion_glmer(Fin_D1_mod) #0.7577132               *****************
plot(residuals(Fin_D1_mod)) + abline(h=0, lty=2) #this looks top heavy                     ****************

#F1 
Fin_F1_mod<- glmer(Infect~Temp+(1|Pot:Symbiont:Temp),data=Fin_F1, family="binomial")
Anova(Fin_F1_mod)
summary(effect(c("Temp"), xlevels=3, Fin_F1_mod))   # what do these pvals coorespond to
dispersion_glmer(Fin_F1_mod) #1.10907
plot(residuals(Fin_F1_mod)) + abline(h=0, lty=2) #Dispersed randomly                      ****************

#G3 

Fin_G3_mod<- glmer(Infect~Temp+(1|Pot:Symbiont:Temp),data=Fin_G3, family="binomial")
Anova(Fin_G3_mod)
summary(effect(c("Temp"), xlevels=3, Fin_G3_mod))   # what do these pvals coorespond to
dispersion_glmer(Fin_G3_mod) #0.5570597
plot(residuals(Fin_G3_mod)) + abline(h=0, lty=2) #Dispersed randomly                      ****************

#All
Fin_C1 <- subset(Fin_C1, select = -c(zscore)) #remove this col
Fin_D1 <- subset(Fin_D1, select = -c(cells.larvae_log)) #remove this col
Fin_G3 <- subset(Fin_G3, select = -c(cells.larvae_log)) #remove this col
Fin_F1 <- subset(Fin_F1, select = -c(cells.larvae_log)) #remove this col
Fin_F1 <- subset(Fin_F1, select = -c(ID)) #remove this col

Day14_infectAll<-rbind(Fin_C1,Fin_D1, Fin_F1, Fin_G3)

##**AH*** why isn't this running right?
Day14_infectAll_infect<- glmer(Infect~Temp*Symbiont+(1|Pot:Symbiont:Temp),data=Day14_infectAll, family="binomial")
Anova(Day14_infectAll_infect)
summary(Day14_infectAll) # what do these pvals coorespond to
dispersion_glmer(Day14_infectAll_infect) #0.8829768
plot(residuals(Day14_infectAll_infect)) + abline(h=0, lty=2) 
...
Organize singles into df with 0s removed
```{r} 
...
###DELETE RAW STUFFF for multi, only relabund
...
YOu might need to get rid of this, it calls on later stuff 
```{r} 
...
#use ALLcast_9000_cast2b which is regular relabund with NAs and 0s removed samples

ALL_RelA<-ALLcast_9000_cast2b #copy
#write.csv(ALLcast_9000_cast2b, "ALLcast_9000_cast2b.csv")
...
ALL_RelAvar<-c("Day", "Temp", "ALL.number","C1","D1","F1" ,"G3")
ALL_RelA_a<-ALL_RelA[ALL_RelAvar]
ALL_RelA_b<-melt(ALL_RelA_a, id=c("Day", "Temp", "ALL.number"), measure.vars = c("C1","D1","F1" ,"G3"))
