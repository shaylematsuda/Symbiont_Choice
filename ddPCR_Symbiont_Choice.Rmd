...
Symbiont choice experiment: uptake of cultured Symbiodiniaceae in no-choice and in a 4-way choice experiments in A. tenuis larvae at ambient (27C) and two elevated (30C and 31C) temperatures over the course of 2 weeks. Sampling was on days 3, 7 and 14.

No-choice infections (single-choice): Individual infection of single species: C1, D1, F1, and G3
4-way choice: Mix of all four species (coded as poly/polyculture)
Pot: the containers larvae were kept in (3 reps/symbiont/temp)
...
#load in df
load("Symbiont_choice_data.RData")


```

# Regression - Fig S2, Tab S1 
Scope/hemo means for symb/temp/time/pot VS ddPCR

Model2 regression: Do ddPCR counts differe significantly from hemocytometer + scope counts?
For infection treatments, we additionally counted symbionts by eye (flurescent scope) on Day 3, and by hemoctyometer (Days 7 and 14) per instrument capacity. Running a model2 analysis, we show that the slope of ddPCR x scope/hemo counts is not different than 1.  
```{r}  
...
#C1Full$cells.larvae_counts_unlog<-(10^C1Full$cells.larvae_log)-1  #converted back to confirm
...
Monoculture<-rbind(C1Full,D1Full, F1Full, G3Full) #no-choice
MonoPoly<-rbind(C1Full,D1Full, F1Full, G3Full,PolyFull) #4-way
...
Mort.df_NEW<-read.csv("Survivorship.csv") #read mortality df
...
#by date: Day 7 
  Mort.df_NEW_D7<-subset(Mort.df_NEW, Day=="7")
##create binomial vector
...
#Full model
Mort.df_NEW_mod_D7<-glmer(x~Temp*Symbiont + (1|Pot), family=binomial)
summary(Mort.df_NEW_mod_D7)
Anova(Mort.df_NEW_mod_D7)
 emm<-emmeans(Mort.df_NEW_mod_D7,  ~Symbiont|Temp, adjust="Tukey")
  cld(emm)
  pairs(emm)

detach(Mort.df_NEW_D7)
  
  #by date: Day 14 
  Mort.df_NEW_D14<-subset(Mort.df_NEW, Day=="14")
##create binomial vector
attach(Mort.df_NEW_D14)
x<-cbind(Alive, Failures)
...
#Full model
Mort.df_NEW_mod_D14<-glmer(x~Temp*Symbiont + (1|Pot), family=binomial)
summary(Mort.df_NEW_mod_D14)
Anova(Mort.df_NEW_mod_D14)
 emm<-emmeans(Mort.df_NEW_mod_D14,  ~Symbiont|Temp, adjust="Tukey")
  cld(emm)
  pairs(emm)

detach(Mort.df_NEW_D14)
``` 
...
# Binomial: Did a larvae infect (Y/N) modeles and plots - Fig 2A, Tab S4
dfs: C1Full, D1Full, F1Full, G3Full, Poly

 
Making binomial dfs
Organize df for 4-way: ddPCR and hemo/scope 
```{r}            
...
Poly_binomial<-rbind(PolyFullscopehemo3,RelA_binaryb_2) #combine for the 4-way BINOMIAL ONLY df
...
#make DF for 4-way cell densities for COUNTS (not binomial) (ALL=Poly/4-way) for below
...
PolyFullscopehemo4<-subset(PolyFullscopehemo2, select = -c(Infect)) #prep scope hemo df
Poly_forCounts<-rbind(PolyFullscopehemo4,ALLcast_9000_cast3) # combine scope/hemo with ddPCR for 4-way
Poly_forCounts<-na.omit(Poly_forCounts) 
```
 
Create binomial for no-choices
```{r}                 
...
Binomial Models
```{r}    
#Full model including all symbiont treatments/temp/time: 
...
#Full model remove day 14 at 31 due to low n  
...
Binary_27_mean <- ddply(Binary_27, c("Symbiont","Day"), summarise, #summarize cell counts
                 N    = length(Infect[!is.na(Infect)]), #calculate the length of the data frame,  excluding NA’s 
...
##### including 31C day 14, all that ampted. Binary_31_31CD13 #####
...
# Binomial: No-choice vs 4-way choice by genus: Binomial (pink and blue plots) - Fig S3, Tab S5  
all amplifications= ALLcast_9000_cast2a 
Plots and comparison models no-choice v 4-way 
...
#No-choice by species (no-choice df binary already, new col for infect type)
...
#Individual Infections of 4-way larvae: only use this data to compare to SINGLE infections. PotCode is coded to keep separate single and multi pots
...
# No-choice and 4-way choice infection: Cell Counts Models and Plots, Table 2, Tab S6
Model: `lmer(Infect~Symbiont*Temp*Day+(1|PotCode))` 
No-choice infections (histograms and full model) 
```{r}      
# poly=4-way choice
#set up the 4-way df for this specific model.
Poly_forCounts$cells.larvae_log<-log10(Poly_forCounts$cells.larvae+1) #log transform
...
CellCounts_no0_NOD1431C_mean <- ddply(CellCounts_no0_NOD1431C, c("Day", "Symbiont","Temp"), summarise,
                  N    = length(cells.larvae[!is.na(cells.larvae)]), 
...
CellCounts_no0_mean <- ddply(CellCounts_no0, c("Day", "Symbiont","Temp"), summarise, #summarize cell counts
                  N    = length(cells.larvae[!is.na(cells.larvae)]), #calculate the length of the data frame
                 mean = mean(cells.larvae, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(cells.larvae, na.rm=TRUE), #calculate standard deviation 
                 se   = sd / sqrt(N), #calculate standard error
                 max = max(cells.larvae, na.rm=TRUE)) #calculate max, could also calculate min () if desired  
...
Poly_no0_27_mean <- ddply(Poly_no0_27, c("Day", "Symbiont"), summarise, #summarize cell counts
                  N    = length(cells.larvae[!is.na(cells.larvae)]), #calculate the length of the data frame,  excluding NA’s 
                 mean = mean(cells.larvae, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(cells.larvae, na.rm=TRUE), #calculate standard deviation 
                 se   = sd / sqrt(N), #calculate standard error
                 max = max(cells.larvae, na.rm=TRUE)) #calculate max, could also calculate min () if desired  
...
``` 
# Plot of Symbiont Aquisition and Densities for all four no-choice and 4-way choice - Figure 2
```{r}   
#log scale axis only   
...
```
# Symbiont density plots by temp by genus - Fig S4 
C1
```{r}      
...
# No-choice vs 4-way choice-infection: Cell Counts (Blue and pink plots), only infected larva - Fig S5
C1            
...
# No-choice vs 4-way: (Pink and Blue Models by symbiont), infected only, SI Tab S7
```{r}            
#stats, log10:  
...
# 4-way infection, % larvae infected with 0,1, 2, 3, or 4 species - Figure 3A   
```{r}        
NumClades<-read.csv("NumberofGeneraperLarva.csv") #Spreadsheet of number of clades each larva infected with by treatments.day.temp
...
NumCladesB_mean <- ddply(NumCladesB, c("Temp","Day","Number"), summarise, #summarize cell count
                 N    = length(NumGenera[!is.na(NumGenera)]), #calculate the length of the data frame, excluding NA’s  
...
# Fig Competative-infection: 4-way only, relative abundance of each species - Figure 3B, Table 4, Tab S11  
...
#all
ALL_RelAb_mean <- ddply(ALL_RelAb_b, c("Day2","Symbiont","Temp"), summarise, #summarize cell counts
                 N    = length(cells.larvae[!is.na(cells.larvae)]), #calculate the length of the data frame, excluding NA’s *this part is important*
                 mean = mean(cells.larvae, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(cells.larvae, na.rm=TRUE), #calculate standard deviation
                 se   = sd / sqrt(N), #calculate standard error
...
# Bar plots of relative abundance by individual larva - Figure 4
```{r}      
aData<-RelA_binaryb
...
```
Fig 4 - stacked bar - INDIVIDUAL LARVAE relative abundance of each genus by time
```{r}     
#27C  
...
# Plot: cells/larvae vs number of species infected per larva - Fig S6, Tab S8
```{r}     
