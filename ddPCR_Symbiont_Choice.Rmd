...
xx = ggplot(data.scores, aes(x = NMDS1, y = NMDS2)) + 
 ...
 # stat_ellipse(level=.5,aes(x=NMDS1, y=NMDS2,color=Temp)) 
  #stat_ellipse(geom="polygon",level=.5,aes(x=NMDS1, y=NMDS2,color=Temp)) 
  stat_ellipse(geom = "polygon",level=.25, alpha = .2, aes(fill=Temp))+
  stat_ellipse(level=.25,  aes(x=NMDS1, y=NMDS2,color=Temp))+

geom_segment(data = data.scores, aes(x = 0, xend=NMDS1, y=0, yend=NMDS2), arrow = arrow(length = unit(0.25, "cm")), colour = "grey10", lwd=0.3) + #add vector arrows of significant env variables
  ggrepel::geom_text_repel(data = data.scores, aes(x=NMDS1, y=NMDS2, label = Day), cex = 4, direction = "both", segment.size = 0.25) #add labels for env variables
