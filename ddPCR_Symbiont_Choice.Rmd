...
https://sebastianraschka.com/Articles/heatmaps_in_r.html
 ...
 Individual clade heatmaps SINGLE INFECTION
```{r}
#C1
#Singles_C1
...
OK you are figuring out if you can make PCA with raw rel abund or should use the transformed rel ablund. 
...
RelA_PCA<-ALL_RelA_a #keep only cols you need
...
# D3 
ALL_RelA_D3<-subset(ALL_RelA, Day=="3")
ALL_RelA_D3<-na.omit(ALL_RelA_D3)
multi.pcaD3 <- prcomp(ALL_RelA_D3[,9:12], scale=F)
multi.pcaD3
summary(multi.pcaD3)
...
# PCA
data_veganDay3 <- RelA_PCA_D3[c(9:12)]
vegan_model_D3 <- scale(data_veganDay3) 

 # PerMANOVA - partitioning the euclidean distance matrix by nutrition & temp
adonisD3<-adonis2(vegan_model_D3 ~ Temp, data = RelA_PCA_D3, method='eu', perm=999)
summary(adonisD3)
adonisD3
...
pca.plotD3 <- autoplot(multi.pcaD3, data = RelA_PCA_D3, colour = 'Temp', shape="Temp",loadings = TRUE, loadings.colour = 'black',loadings.label = TRUE, loadings.label.colour="black", loadings.label.size = 5, loadings.label.vjust=-1, frame.type="norm", frame=TRUE)+
  ...
  xlim(-0.3,0.5) +
  ylim(-0.3, 0.4) +
  geom_text(x=0.1, y=0.3, label="PERMANOVA p(Temp)<0.01", size=5)+
  theme(legend.text = element_text(size=18), 
        plot.background = element_blank(),
        legend.title = element_text(size=18, face="bold"), 
        axis.text = element_text(size=18), 
...
#spca.plot$layers[[2]]$aes_params$size <- 1
#ggsave("Figures/PCA_D3_20200618.pdf", plot=pca.plotD3, height=8, width=12, units = c("in"), dpi=300) #output figure 
```
...
# D7 
ALL_RelA_D7<-subset(ALL_RelA, Day=="7")
ALL_RelA_D7<-na.omit(ALL_RelA_D7)
multi.pcaD7 <- prcomp(ALL_RelA_D7[,9:12], scale=F)
multi.pcaD7
summary(multi.pcaD7)
...
# PCA
data_veganDay7 <- ALL_RelA_D7[c(9:12)]
vegan_model_D7 <- scale(data_veganDay7) 

 # PerMANOVA - partitioning the euclidean distance matrix by nutrition & temp
adonisD7<-adonis2(vegan_model_D7 ~ Temp, data = RelA_PCA_D7, method='eu', perm=999)
summary(adonisD7)
adonisD7
...
```
## Individual clade heatmaps MUTLI INFECTION counts not relabund
