...
#Mort.df_NEW_D7<-Mort.df_NEW[which(Mort.df_NEW$Day == "7"),] #subset d7 timepoint
#Mort.df_NEW_D14<-Mort.df_NEW[which(Mort.df_NEW$Day == "14"),] #subset d14 timepoint


#Summary of total survivorship at the end of the thermal stress test. Display survival as percentages (%).     
#D7.Summary <- ddply(Mort.df_NEW_D7, c("Temp", "Symbiont"), summarise,
...
#knitr::kable(D7.Summary)

#D14.Summary <- ddply(Mort.df_NEW_D7, c("Temp", "Symbiont"), summarise,
#                       N    = length(Surv[!is.na(Surv)]),
#                        mean = mean(Surv, na.rm=TRUE)*100,
#                        sd   = sd(Surv, na.rm=TRUE)*100,
#                        se   = sd / sqrt(N)
...
y<-cbind(Alive, Failures)

Mort.df_NEW_MvP_mod<-glmer(y~Temp*MonoPoly*Day + (1|Pot), family=binomial) #why isn't this running? I think it's in the "y"?
...
#grid.arrange(CountsCompare2_D1_plot27, CountsCompare2_D1_plot30, CountsCompare2_D1_plot31, nrow=1)
#
...
CountsCompare2_C1_mod<-lmer(cells.larvae_log~Temp*Day*single.multi+(1|PotCode),CountsCompare2_C1_27C30C)
anova(CountsCompare2_C1_mod)
qqPlot(residuals(CountsCompare2_C1_27C30C))   # good
  emm<-emmeans(CountsCompare2_C1_27C30C,  ~ single.multi|Temp|Day, adjust="Tukey") 
cld(emm)
pairs(emm)
...
#model just for D3 
CountsCompare2_C1_D3<-subset(CountsCompare2_C1, Day=="3")
CountsCompare2_C1_D3_mod<-lmer(cells.larvae_log~Temp*single.multi+(1|PotCode),CountsCompare2_C1_D3)
anova(CountsCompare2_C1_D3_mod)
qqPlot(residuals(CountsCompare2_C1_D3_mod))   # good
  emm<-emmeans(CountsCompare2_C1_D3_mod,  ~ single.multi|Temp|Day, adjust="Tukey") 
cld(emm)
pairs(emm)
...
CountsCompare2_D1_mod<-lmer(cells.larvae_log~Temp*Day*single.multi+(1|PotCode),CountsCompare2_D1_27C30C)
anova(CountsCompare2_D1_mod)
qqPlot(residuals(CountsCompare2_D1_mod))   # good
  emm<-emmeans(CountsCompare2_D1_mod,  ~ single.multi|Temp|Day, adjust="Tukey") 
cld(emm)
...
CountsCompare2_G3_mod<-lmer(cells.larvae_log~Temp*Day*single.multi+(1|PotCode),CountsCompare2_G3_27C30C)
anova(CountsCompare2_G3_mod)
qqPlot(residuals(CountsCompare2_G3_mod))   # good
  emm<-emmeans(CountsCompare2_G3_mod,  ~ single.multi|Temp|Day, adjust="Tukey") 
cld(emm)
