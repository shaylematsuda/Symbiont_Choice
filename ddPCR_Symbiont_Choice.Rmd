...
Countdat_2_d3<-subset(Countdat_2, Day=="3")
...
C1Full$PotCode<-paste0(C1Full$Temp, C1Full$Day, C1Full$Pot, C1Full$count.type)
...
#ALL
#organize the ddPCR data 
ALL_vars<-c("Temp","Day", "Info","Pot", "Replicate","cells.larvae")
ALL_9000b<-ALL_9000[ALL_vars]
...
AScopehemo_ddPCRCompare<- merge(scopehemoFull_mean2,ddPCRcompare_mean2,by=c("Symbiont","Temp","Day","Pot"))
...
ConversionCheck<-rbind(C1Full,D1Full, F1Full, G3Full, ALLFull)
  plot(ConversionCheck$cells.larvae, ConversionCheck$cells.larvae_counts_unlog) #check
...
```{r}       
BinaryInfect<-ConversionCheck  # make new df for binary infection
...
#Full model including all clades/temp/time:
 Binary_mod<-glmer(Infect~Symbiont*Temp*Day+(1|PotCode), family="binomial", BinaryInfect) #
  Anova(Binary_mod)
dispersion_glmer(Binary_mod) # no overdispersion  .97
  emm<-emmeans(Binary_mod,  ~ Symbiont|Temp)
  pairs(emm)
  cld(emm) 
...
dispersion_glmer(BinaryInfect_27and30_mod) # .95
  emm<-emmeans(BinaryInfect_27and30_mod,  ~ Symbiont|Temp)
  pairs(emm)
  cld(emm) 
...
dispersion_glmer(BinaryInfect_D3andD7_mod) # no overdispersion  1.01
  emm<-emmeans(BinaryInfect_D3andD7_mod,  ~ Symbiont|Temp)
  pairs(emm)
  cld(emm) 
```


By clade: infection ~ Temp*Day (keep bc you need dfs)
```{r} 
#by clade:   
#C1
...
# Monoculture vs Polyculture by clade: Binomial TAKE 2 -PINK and BLUE binomial:all amplifications ALLcast_9000_cast2a
...
RelA_binaryb$numclade<-RelA_binaryb$C1+RelA_binaryb$D1+RelA_binaryb$F1+RelA_binaryb$G3

#write.csv(RelA_binaryb,"RelA_binaryb20200820.csv")


#GIVE TO AH to write if else to make this tell you if an ALL larva infected even if NAs for some clades. this is CONSERVATIVE. 
...
RelA_binaryb$Infect[RelA_binaryb$numclade==0]<-0
   #RelA_binaryb$Infect<-as.factor(RelA_binaryb$binary) #make factor
...
#df for multi:RelA_binaryb, df for Individuals:  BinaryInfect
 RelA_binaryb$Symbiont<-"All"
  
RelA_binaryb_ALL<-RelA_binaryb[,c("Temp","Day","PotCode","Infect","Symbiont")] #keep only cols you need
binarybInfect_ALL<-BinaryInfect[,c("Temp","Day","PotCode","Infect","Symbiont")] #keep only cols you need
Multi_Single_binaryb<-rbind(RelA_binaryb_ALL,binarybInfect_ALL) #combine
 Multi_Single_binaryb$Symbiont<-as.factor(Multi_Single_binaryb$Symbiont)
  Multi_Single_binaryb$Infect<-as.factor(Multi_Single_binaryb$Infect)

  
#Full model including all clades: You dont need this
 Multi_Single_binaryb_model<-glmer(Infect~Temp*Symbiont*Day+(1|PotCode), family="binomial", Multi_Single_binaryb) 
 Anova(Multi_Single_binaryb_model)
dispersion_glmer(Multi_Single_binaryb_model) # .79
  emm<-emmeans(Multi_Single_binaryb_model,  ~ Symbiont*Temp*Day)
  pairs(emm)
  cld(emm)  
