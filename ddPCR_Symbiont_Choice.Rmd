...
#FIG SI1:Do ddPCR counts differe significantly from hemocytometer + scope counts?
...
#Single-genus Infection: Models and plots: Binomial: Did a larvae infect (Y/N)
...
#Single-genus infection_ZEROS REMOVED: Models, log transformed
Model: `lmer(Infect~Symbiont*Temp*Day+(1|PotCode))`
using log transformed! (Cells.larvae_log)  
Single infections (histograms and full model) 
```{r}       
...
#Single-genus Infection: ZEROS REMOVED Plots: by clade,cell counts (count data, with 0s included) 
C1 single infection plot  
```{r}         
...
#Single-genus Infection: Fig 1: Heat maps, by clade (temp x time)  
...
#***Single-genus & Competative-infection:counts PLOTS (Blue and pink plots) with 0s removed for multi (aka ONLY when they infected with a SPECIFIC zoox)
C1      
C1Full2 = Single cells.larvae raw
ALLcast_9000_cast2a = Alls raw that infected  EDIT THIS per each clade to remove 0s
```{r}     
...
#***Single-genus & Competative-infection: Models by symbiont, 0s removed for multi within specific zoox. 
```{r}   
#stats, log10: Normal 
...
qqPlot(residuals(CountsCompare2_G3_mod))   # ok
...
#Single-genus & Competative-infection: Models by symbiont (0s included still)
```{r}   
#stats, log10
...
#Single-genus & Competative-infection:counts PLOTS (Blue and pink plots)
C1    
C1Full2 = Single cells.larvae raw
