...
# Larval Mortality
```{r}       
# Monos and the poly (remove control)
...
#remove day1 and remove COntrols (no rep)
D.Summary<-subset(D.Summary, Day==7|Day==14)

#remove control due to no replication 
...
#monoculture only model
Mort.df_NEW_mono<-subset(Mort.df_NEW, Symbiont=="C1"|Symbiont=="D1"|Symbiont=="F1"|Symbiont=="G3")

##create binomial vector
attach(Mort.df_NEW_mono)
...
#ALL model
Mort.df_NEW_mono_mod<-glmer(y~Temp*Symbiont*Day + (1|Pot), family=binomial)
summary(Mort.df_NEW_mono_mod)
Anova(Mort.df_NEW_mono_mod)

#Tukey posthoc
 emm<-emmeans(Mort.df_NEW_mono_mod,  ~ Symbiont|Temp|Day, adjust="Tukey")
cld(emm, Letter="abcdefg")
pairs(emm)

 emm<-emmeans(Mort.df_NEW_mono_mod,  ~ Temp|Day*Symbiont, adjust="Tukey")
cld(emm, Letter="abcdefg")
pairs(emm)

detach(Mort.df_NEW_mono)
...
Make binomial dfs
Organize df for Polyculture: ddPCR and hemo./scope
```{r}      
#ALLcast_9000_cast2a includes all good ddpcrs even if not all genera amped for each larva
 
#polyculture df: create binary df, if at least one genus had at least 1 cell, designate "1"
#organize the ddPCR data 
...
Poly_forCounts<-na.omit(ALL_forCounts) 
```
...
BinaryInfect<-na.omit(BinaryInfect) #remove nas
 ...
 ##########WHY ISN"T IT WORKING NOW. I literally didn't change anything. ugh. Individual genera work and the 27+30C only works
 Binary_mod<-glmer(Infect~Temp*Day*Symbiont+(1|PotCode), family="binomial", BinaryInfect)
  Anova(Binary_mod)
...
BinaryInfect_C1<-subset(BinaryInfect, Symbiont=="C1")
 Binary_mod_C1<-glmer(Infect~Temp*Day+(1|PotCode), family="binomial", BinaryInfect_C1) #
  Anova(Binary_mod_C1)
dispersion_glmer(Binary_mod_C1) # no overdispersion  .77, also this no longer runs thanks to R update issue with package.
...
# Binomial: Monoculture vs Polyculture by clade: Binomial TAKE 2 -PINK and BLUE binomial: 20200824  
