...
#you are trying this new again (old is saved below incase) 20200824
...
#write.csv(NumCladesB_mean,"NumCladesB_mean.csv")
...
Clade winners: which genus was dominant in larvae on avg in each infection number group?
```{r}
...
6<-dcast(T30_D14_rel, Temp+Day+NumGenera~NumberWinners)
...
T31_D14_rel<-T31_D14 %>% group_by(NumGenera) %>% mutate(relAbundByPath = numblarv / sum(numblarv))
T31_D14_rel<-subset(T31_D14_rel, select = -c(numblarv) )
names(T31_D14_rel)[names(T31_D14_rel)=="relAbundByPath"] <- "RelA"
...
aData4_3or4_27_D14_g4_plot<-ggplot(aData4_3or4_27_D14_g4, aes(y=cells.larvae, x=ALL.number, fill=Symbiont),width=0.5) + 
 ...
 theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) +
  ggtitle("27C_D14_4gen");aData4_3or4_27_D3_g4_plot
...
# Competative-infection: Polycluture only, relative abundance of each clade  
...
##Subset Day 3
ALL_RelA_b_D3<-subset(ALL_RelA_b, Day=="3")

ALL_RelA_b_D3_mod<-lmer(cells.larvae_log~Symbiont*Temp+(1|ALL.number)+(1|Pot), ALL_RelA_b_D3)
anova(ALL_RelA_b_D3_mod)
qqPlot(residuals(ALL_RelA_b_D3_mod))   # meh
...
****NEW Plot cells/larvae vs number of clades infected - shows that as more cells infected more clades infected
```{r}
...
CellsByNum_plot27<-ggplot(data=CellsByNum_27C, aes(x=numclade, y=cells.larvae_log, color=Day)) +
  ...
  geom_point( size=4, show.legend = T)+ 
      ...
      # geom_point(shape = 1,size = 4,colour = "black")+
  #geom_line(linetype="dotted")+
  xlab("number of clades infected by") + #Label the X Axis 
  #ylim(0, 1) + #set Y limits 
       #  scale_x_continuous(breaks=seq(0, 14, 2), limits =c(0,14))+
  theme_bw() + #Set the background color
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        plot.background =element_blank(), #Set the plot background
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste("cells larva log"))) +
...
#MODELS
CellsByNum_model<-lmer(numclade ~cells.larvae_log*Day*Temp+(1|Pot), CellsByNum)
