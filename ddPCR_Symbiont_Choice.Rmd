...
#For defense then cut
```{r}
BinaryInfectAll<-BinaryInfect
BinaryInfectAll$Infect<-as.numeric(as.character(BinaryInfectAll$Infect))
BinaryInfectAll_mean <- ddply(BinaryInfectAll, c("Symbiont","Day","Temp"), summarise, #summarize cell counts
                 N    = length(Infect[!is.na(Infect)]), #calculate the length of the data frame,  excluding NA’s *this part is important*
                 mean = mean(Infect, na.rm=TRUE), #calculate mean of response variable, removing NA's
                 sd   = sd(Infect, na.rm=TRUE), #calculate standard deviation 
                 se   = sd / sqrt(N), #calculate standard error
...
BinaryInfectAll_mean #display table
...
#Binary_31_31CD13_mean<-Binary_31_31CD13_mean[!(Binary_31_31CD13_mean$N=="3"),]
 ...
 #change Day back to Numeric from Factor, reapply the actual numeric dates
BinaryInfectAll_mean$Day.num<-as.numeric(BinaryInfectAll_mean$Day)
BinaryInfectAll_mean$Day.num[BinaryInfectAll_mean$Day.num == 2] =7
BinaryInfectAll_mean$Day.num[BinaryInfectAll_mean$Day.num == 3] =14
BinaryInfectAll_mean$Day.num[BinaryInfectAll_mean$Day.num == 1] =3

BinaryInfectAll_mean<-subset(BinaryInfectAll_mean, Symbiont=="C1"|Symbiont=="F1")

Binarymean_plot<-ggplot(data=BinaryInfectAll_mean, aes(x=Day.num, y=mean, group = Symbiont, color=Symbiont)) +
  geom_errorbar(aes(ymin=mean-se, ymax=mean+se),
                width=0.1, show.legend = T)+
  geom_point(aes(color=Symbiont), size=8, show.legend = T)+ 
  geom_line(aes(color=Symbiont))+
  scale_color_manual(values=c('#D43F3AFF','#5CB85CFF'))+
  ...
  theme(axis.line = element_line(color = 'black'), #Set the axes color
        ...
        legend.key = element_blank()) + #Set plot legend key
  ylab(expression(paste(""))) + 
 ggtitle("Infection Success")+ 
  theme(plot.title = element_text(size=20, face = "italic", hjust=0.5));Binarymean_plot
Binarymean_plot+facet_wrap(~Temp)
